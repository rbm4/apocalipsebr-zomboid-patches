...
public class WorldMapVisitedServer {
    ...
    public void loadUser(IConnection connection) {
        if (!this.dictionary.containsKey(connection.getUserName())) {
            File oldFile = new File(this.getFolderName() + connection.getUserName());
            File zipFile = new File(this.getFolderName() + connection.getUserName() + ".zip");
            ...
            if (zipFile.exists()) {
                try (
                    ...
                    InputStream iS = Files.newInputStream(zipFS.getPath(connection.getUserName()));
    ...
    public void unloadUser(String user) {
        if (this.dictionary.containsKey(user)) {
            File zipFile = new File(this.getFolderName() + user + ".zip");
            Path zipPath = FileSystems.getDefault().getPath(zipFile.getPath()).toAbsolutePath();
            Map<String, String> env = new HashMap<>();
            env.put("create", String.valueOf(Files.notExists(zipPath)));
            ...
            try (
                ...
                OutputStream oS = Files.newOutputStream(
                    zipFS.getPath(user), StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING
                );
    ...
    public void deleteUser(String user) {
        File file = new File(this.getFolderName() + user + ".zip");
        File oldFile = new File(this.getFolderName() + user + ".zip");
        if (file.exists()) {
            file.delete();
        }
