    ...
    private void DrawTextRotated(
        ...
        if (sdfShader.getShaderProgram().isCompiled()) {
            ...
            if (this.isUserDefined() && this.isShared() && drawer.renderer.isDimUnsharedSymbols() && layout.isAuthorHidden) {
                VBORenderer vborx = VBORenderer.getInstance();
                vborx.startRun(vborx.formatPositionColor);
                vborx.setMode(1);
                this.getOutlinePoints(drawer, leftTop, rightTop, rightBottom, leftBottom);
                vborx.addLine((float)rightTop.x, (float)rightTop.y, 0.0F, (float)leftBottom.x, (float)leftBottom.y, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F);
                vborx.addLine((float)leftTop.x, (float)leftTop.y, 0.0F, (float)rightBottom.x, (float)rightBottom.y, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F);
                vborx.endRun();
            }
