...
public final class WorldMapClient {
    ...
    private void receiveAddSymbol(ByteBufferReader bb) throws IOException {
        if (this.receivedRequestData) {
            SymbolSaveData saveData = new SymbolSaveData(247, 2);
            saveData.load(bb.bb);
            WorldMapSymbolNetworkInfo networkInfo = new WorldMapSymbolNetworkInfo();
            networkInfo.load(bb.bb, 247, 2);
    ...
    private void receiveModifySymbol(ByteBufferReader bb) throws IOException {
        if (this.receivedRequestData) {
            ...
            if (symbol != null) {
                symbol.getNetworkInfo().load(bb.bb, 247, 2);
                symbol.load(bb.bb, saveData);
                if (this.worldMapLoaded) {
    ...
    public void setAuthorHidden(String userName, boolean hidden) {
        HiddenAuthors.setAuthorHidden(GameClient.serverName, userName, hidden);
        HiddenAuthors.write();
    }

    public boolean isAuthorHidden(String userName) {
        return HiddenAuthors.isAuthorHidden(GameClient.serverName, userName);
    }
