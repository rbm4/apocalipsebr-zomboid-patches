...
public final class CombatManager {
    ...
    private HitInfo calcValidTarget(IsoLivingCharacter owner, HandWeapon weapon, IsoMovingObject mov, float range) {
        if (mov == owner) {
            ...
            if (mov.getCurrentSquare() == null) {
                return null;
            } else if (isoGameCharacter.isGodMod() && !isoGameCharacter.isZombie()) {
                return null;
    ...
    public static boolean checkPVP(IsoMovingObject owner, IsoMovingObject target) {
        ...
        if (PZOptional.ifPresent(targetPlayer, false, IsoGameCharacter::isGodMod)) {
            ...
            return true;
        } else if (!(player instanceof IsoAnimal) && !(targetPlayer instanceof IsoAnimal)) {
            if (GameClient.client) {
                ...
                if (!player.factionPvp && !targetPlayer.factionPvp) {
    ...
    private boolean removeTargetObjects(IsoMovingObject obj, IsoGameCharacter owner) {
        if (obj instanceof IsoGameCharacter gameCharacter && (gameCharacter.isDead() || gameCharacter.isGodMod() && !gameCharacter.isZombie())) {
            ...
            return obj.getCurrentSquare() == null ? true : !checkPVP(owner, obj);
        }
    }
    ...
    private void filterTargetsByZ(float ownerZ, PZArrayList<HitInfo> hitList) {
        ...
        if (hitInfoMinDeltaZ != null) {
            for (int ix = hitList.size() - 1; ix >= 0; ix--) {
                HitInfo hitInfo = hitList.get(ix);
