...
public class NetworkZombieAI extends NetworkCharacterAI {
    ...
    private void setUsingWalkTowardState(ZombiePacket packet) {
        if (this.zombie.getPath2() == null) {
            ...
            if (length > 5.0F) {
                ...
                packet.z = (byte)PZMath.fastfloor(this.pfb2.getTargetZ());
    ...
    private boolean checkZTeleport(IsoZombie zombie) {
        if (PZMath.fastfloor(zombie.getZ()) == zombie.realz) {
            ...
            IsoGridSquare square = zombie.getCell().getGridSquare((double)zombie.realx, (double)zombie.realy, (double)zombie.realz);
