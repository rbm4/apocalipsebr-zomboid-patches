...
public final class AnimalPopulationManager {
    ...
    public int removeChunkFromWorld(IsoChunk chunk) {
        if (this.client || chunk == null) {
            return 0;
        }
        ...
        try {
                ...
                for (int z = chunk.getMinLevel(); z <= chunk.getMaxLevel(); z++) {
                    for (int y = 0; y < 8; y++) {
                        for (int x = 0; x < 8; x++) {
                            ...
                            if (sq == null || sq.getMovingObjects() == null || sq.getMovingObjects().isEmpty()) {
                                continue;
                            }

                            ArrayList<IsoMovingObject> snapshot = new ArrayList<>();
                            try {
                                for (int i = 0; i < sq.getMovingObjects().size(); i++) {
                                    snapshot.add(sq.getMovingObjects().get(i));
                                }
                            } catch (Throwable t) {
                                skipped++;
                            ...
                            for (int i = 0; i < snapshot.size(); i++) {
                                IsoMovingObject movingObject = snapshot.get(i);
            ...
            ApocBRServerTelemetry.recordAnimalUnloadVirtualization(
                    seen, virtualized, skipped, System.nanoTime() - startNanos);
        }
    }
