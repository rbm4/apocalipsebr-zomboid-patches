    ...
    public void requestSaveCell(int popmanCellX, int popmanCellY) {
        if (!GameClient.client) {
            ...
            try {
                ...
                // ApocBR: requestSaveCell can run from async chunk retirement.
                // Snapshot the live zombie list before scanning it so concurrent
                // main-thread add/remove operations do not trip ArrayList's
                // fail-fast iterator and abort the cell save request.
                List<IsoZombie> zombies = new ArrayList<>(IsoWorld.instance.currentCell.getZombieList());

                for (IsoZombie z : zombies) {
                    // Async chunk retirement can observe a transient empty slot
                    // while the main simulation moves/reuses a zombie. This
                    // server accepts dropping that transient entry; a missing
                    // IsoZombie has no position or Z axis that can be saved.
                    if (z == null) {
                        continue;
                    }
