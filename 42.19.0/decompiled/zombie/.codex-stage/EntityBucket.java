...
public abstract class EntityBucket {
    ...
    final void updateMembership(GameEntity entity) {
        // ApocBR: async entity/chunk retirement can leave a delayed bucket update
        // targeting an entity whose component container was already released. In
        // that state there are no bucket bits to read or publish, so skip it
        // instead of spamming debug errors or throwing in Core.debug.
        if (entity == null || !entity.hasComponentData()) {
            return;
        }
