...
public final class EngineEntityManager {
    ...
    void processPendingOperations() {
        // ApocBR: async chunk/cell retirement can mutate entity operation state
        // while the engine drains pending work. Snapshot the operation list and
        // skip transient null entities so ObjectSet never receives a null key.
        EngineEntityManager.EntityOperation[] operations = this.pendingOperations.toArray(EngineEntityManager.EntityOperation.class);
        this.pendingOperations.clear();

        for (EngineEntityManager.EntityOperation operation : operations) {
            ...
            switch (operation.type) {
                ...
                case RemoveAll:
                    if (operation.entities != null) {
                        GameEntity[] snapshot = operation.entities.toArray(GameEntity.class);
                        for (GameEntity entity : snapshot) {
                            if (entity != null) {
                                this.removeEntityInternal(entity);
                            }
