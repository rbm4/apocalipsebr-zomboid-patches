...
public class InventoryItem extends GameEntity {
    ...
    protected static final float TOWEL_WET_SERVER_TIME_DELTA = 5.0F;
    ...
    public void DoTooltipEmbedded(ObjectTooltip tooltipUI, ObjectTooltip.Layout layoutOverride, int offsetY) {
        ...
        if (this.gunType != null) {
            ...
            itemx.setValue(this.getGunTypeString(), 1.0F, 1.0F, 1.0F, 1.0F);
        }
        ...
        if (this.hasTag(ItemTag.COMPASS) && this.isInPlayerInventory()) {
            ...
            itemx = layout.addItem();
            itemx.setLabel(Translator.getText(COMPASS_TOOLTIP.get(dir)), 1.0F, 1.0F, 0.8F, 1.0F);
        }
    ...
    private void soakTowel() {
        float multiplier = GameServer.server ? GameTime.getInstance().getMultiplierFromTimeDelta(5.0F) : GameTime.getInstance().getMultiplier();
        this.setWetCooldown(this.getWetCooldown() + RainManager.getRainIntensity() * (4.0F * multiplier));
        if (GameServer.server) {
            GameServer.sendItemStats(this);
        }
    }
    ...
    public ClothingItem getClothingItem() {
        ...
        } else if ("LeftHand".equalsIgnoreCase(alternateModelName1)) {
