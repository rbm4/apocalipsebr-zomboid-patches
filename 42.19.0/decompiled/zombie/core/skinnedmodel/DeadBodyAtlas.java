        ...
        void calcModelBounds(float[] bounds) {
            ...
            if (bRagdoll) {
                ...
                for (int ix = 0; ix < this.body.diedBoneTransforms.length; ix++) {
                    if (!bIgnoreBone[ix]) {
                        worldSpace[ix].rotate((float) (Math.PI / 2), xAxis);
                        worldSpace[ix].rotate((float) Math.PI, yAxis);
                    }
                }
            }

            for (int ixx = 0; ixx < this.animatedModel.getAnimationPlayer().getModelTransformsCount(); ixx++) {
                if ((!bRagdoll || !bIgnoreBone[ixx] && ixx != dummy01 && ixx != bip01 && ixx != bip01Pelvis) && ixx != translationData) {
