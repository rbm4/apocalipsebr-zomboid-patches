...
public class IsoDoor extends IsoObject implements BarricadeAble, Thumpable, IHasHealth, ILockableDoor, ICurtain {
        ...
        if (chr != null && !(chr instanceof IsoAnimal)) {
            ...
            if (this.isBarricaded()) {
                ...
                if (this.locked && !bUnlock && !this.isOpen()) {
                    ...
                    boolean wasOpenx = this.isOpen();
                    toggleDoubleDoor(this, true);
                    if (wasOpenx != this.isOpen()) {
                        this.playDoorSound(chr.getEmitter(), this.isOpen() ? "Open" : "Close");
                        if (player.isLocalPlayer()) {
                            player.triggerMusicIntensityEvent(this.isOpen() ? "DoorOpen" : "DoorClose");
                        }
                    ...
                    if (GameServer.server) {
                        for (UdpConnection connectionx : GameServer.udpEngine.connections) {
                            if (connectionx.isFullyConnected()
                                && connectionx.isRelevantTo(this.getX(), this.getY())
    ...
    public String getThumpSound() {
        ...
        if (!StringUtils.isNullOrWhitespace(soundName)) {
            ...
            String var2 = this.getSoundPrefix();
    ...
    public static void forEachDoorObject(IsoObject object, Consumer<IsoObject> consumer) {
        if (getDoubleDoorIndex(object) != -1) {
            for (int ddIndex = 1; ddIndex <= 4; ddIndex++) {
                IsoObject var4 = getDoubleDoorObject(object, ddIndex);
                if (var4 instanceof IsoObject) {
