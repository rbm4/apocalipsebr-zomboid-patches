...
public class FluidContainer extends Component {
    ...
    public String getUiName() {
        if (this.cacheInvalidated || this.nameCache == null) {
            ...
            if (visibleFluids.isEmpty()) {
                ...
                FluidInstance fluidInstanceAx = visibleFluids.get(0);
                FluidInstance fluidInstanceBx = visibleFluids.get(1);
                FluidInstance fluid = fluidInstanceAx.getAmount() < fluidInstanceBx.getAmount() ? fluidInstanceAx : fluidInstanceBx;
                this.nameCache = Translator.getText("Fluid_HoldingOneType", containerName, fluid.getFluid().getTranslatedName());
                if (!fluidInstanceAx.getTranslatedName().equals(fluidInstanceBx.getTranslatedName())) {
                    FluidInstance fluidB = fluidInstanceAx.getAmount() < fluidInstanceBx.getAmount() ? fluidInstanceBx : fluidInstanceAx;
                ...
                for (FluidInstance fluidInstancex : visibleFluids) {
                    if (!fluidInstancex.getFluid().isCategory(FluidCategory.Beverage)) {
                        allBeverages = fluidInstancex.getFluid().isCategory(FluidCategory.Poisons);
                    }

                    if (fluidInstancex.getFluid().isCategory(FluidCategory.Beverage) && fluidInstancex.getFluid().isCategory(FluidCategory.Alcoholic)) {
                        alcoholics++;
                    }
                }
    ...
    private void recalculateCaches(boolean force, boolean removeInvalid) {
        if (this.cacheInvalidated || force) {
            ...
            if (this.amountCache > 0.0F) {
                for (FluidInstance fluidInstance : this.fluids) {
                    fluidInstance.setPercentage(fluidInstance.getAmount() / this.amountCache);
                    r += fluidInstance.getColor().r * fluidInstance.getPercentage();
                    g += fluidInstance.getColor().g * fluidInstance.getPercentage();
                    b += fluidInstance.getColor().b * fluidInstance.getPercentage();
                    if (fluidInstance.getFluid().getProperties() != null) {
                        float multi = this.amountCache * fluidInstance.getPercentage();
                        this.propertiesCache.addFromMultiplied(fluidInstance.getFluid().getProperties(), multi);
                    }
                }
            }
    ...
    private float getPoisonAmount() {
        ...
        for (FluidInstance fluidInstance : this.fluids) {
            if (fluidInstance.getFluid().isPoisonous()) {
                poisonAmount += fluidInstance.getAmount();
            }
        }
    ...
    public PoisonEffect getPoisonEffect() {
        ...
        for (FluidInstance fluidInstance : this.fluids) {
            if (fluidInstance.getFluid().getPoisonInfo() != null) {
                PoisonInfo poison = fluidInstance.getFluid().getPoisonInfo();
                PoisonEffect poisonEffect = poison.getPoisonEffect(this.getAmount() * fluidInstance.getPercentage(), fluidInstance.getPercentage());
    ...
    public boolean canAddFluid(Fluid fluid) {
        if ((this.fluids.size() < 8 || this.contains(fluid)) && !this.inputLocked && this.canPlayerEmpty) {
            for (FluidInstance instance : this.fluids) {
                if (!instance.getFluid().canBlendWith(fluid) || !fluid.canBlendWith(instance.getFluid())) {
                    return false;
                }
            }
    ...
    private void addFluid(FluidInstance addInstance, float add) {
        ...
        if (add > 0.0F && !this.isFull() && this.canAddFluid(addInstance.getFluid())) {
            ...
            if (this.getOwner() instanceof InventoryItem item && item.getContainer() != null && item.getContainer().getParent() instanceof IsoPlayer) {
                item.syncItemFields();
            }
    ...
    public boolean isCategory(FluidCategory category) {
        for (FluidInstance fluid : this.fluids) {
            if (fluid.getFluid().isCategory(category)) {
                return true;
            }
        }
    ...
    public void save(ByteBuffer output) throws IOException {
        ...
        if (!this.fluids.isEmpty()) {
            ...
            if (this.fluids.size() == 1) {
                ...
                for (FluidInstance fluid : this.fluids) {
                    FluidInstance.save(fluid, output);
                }
            }
        }
