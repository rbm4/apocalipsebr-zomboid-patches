    ...
    public void possiblyPlay(IsoGameCharacter character, float x, float y, float z, int hitLocation, float absSpeedKmHour) {
        ...
        if (slot != null) {
            if (slot.time <= millis - 500L) {
                ...
                emitter.setParameterValue(soundRef, FMODManager.instance.getParameterDescription("VehicleHitLocation"), hitLocation);
                emitter.setParameterValue(soundRef, FMODManager.instance.getParameterDescription("VehicleSpeed"), absSpeedKmHour);
                slot.time = millis;
            }
        }
    }
