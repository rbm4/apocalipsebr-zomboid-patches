...
public enum AntiCheat {
    ...
    Recipe(AntiCheatRecipe.class, ServerOptions.instance.antiCheatChecksum, 1),
    RecipeUpdate(AntiCheatRecipeUpdate.class, ServerOptions.instance.antiCheatChecksum, 4),
    Checksum(AntiCheatChecksum.class, ServerOptions.instance.antiCheatChecksum, 1),
    ChecksumUpdate(AntiCheatChecksumUpdate.class, ServerOptions.instance.antiCheatChecksum, 4),
    ...
    public boolean isValid(UdpConnection connection, INetworkPacket packet) {
        if (GameServer.server && this.isEnabled()) {
            ...
            if (result != null) {
                ...
                if ("".equals(result)) {
                    String message = String.format("Anti-Cheat %s skipped", this.anticheat.getClass().getSimpleName());
                    DebugType.Multiplayer.warn(message);
                    EventManager.instance().report("[" + connection.getUserName() + "] " + message);
                    log(connection, this, connection.getValidator().getCounters().getOrDefault(this, 0), "'skip'");
                } else {
    ...
    public static void preUpdate(UdpConnection connection) {
        for (AntiCheat antiCheat : values()) {
            if (!antiCheat.anticheat.preUpdate(connection)) {
                DebugType.Multiplayer.warn("AntiCheat %s pre-update check failed", antiCheat.name());
            }
        }
    }
        ...
        if (antiCheat.isEnabled()) {
            try {
                String message = String.format(
                    "Warning: player=\"%s\" server-option=%s anti-cheat=%s reason=%s counter=%d/%d action=%s",
