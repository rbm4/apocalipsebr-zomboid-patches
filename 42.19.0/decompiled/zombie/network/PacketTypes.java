...
public class PacketTypes {
    ...
    public static class PacketAuthorization {
        ...
        public static void onUnauthorized(UdpConnection connection, PacketTypes.PacketType type) {
            AntiCheat.Capability.act(connection, "Packet/" + type.name());
        }
    }
        ...
        public void send(IConnection connection) {
            if (GameClient.client && connection.isLimitExceeded(this)) {
                ...
                if (this.isLogEnabled()) {
                    DebugType.Packet.debugln("%s %s", connection.toString(), connection.getPacket(this).getDescription());
                }
            }
        }
                ...
                if ((this.handlingType & 1) != 0) {
                    if (this.handler != null) {
                        ...
                        if (!packet.isConsistent(connection)) {
                            DebugType.Multiplayer.warn("The packet %s is not consistent: %s", this.name(), packet.getDescription());
                            packet.sync(this, connection);
                            return;
                        }

                        if (this.isLogEnabled()) {
                            DebugType.Packet.debugln("%s %s", connection.toString(), packet.getDescription());
                        }
        ...
        public void onClientPacket(ByteBufferReader bb) throws Exception {
            ...
            if ((this.handlingType & 2) != 0) {
                if (this.handler != null) {
                    ...
                    if (this.isLogEnabled()) {
                        DebugType.Packet.debugln("%s %s", GameClient.connection.toString(), packet.getDescription());
                    }
