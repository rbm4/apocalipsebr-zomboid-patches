...
public class PlayerXpPacket extends PlayerID implements INetworkPacket {
    ...
    public void write(ByteBufferWriter b) {
        ...
        try {
            ...
            DebugType.Multiplayer.printException(var3, "Player XP save error", LogSeverity.Error);
        }
    }
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        if (this.isConsistent(connection) && !this.getPlayer().isDead()) {
            try {
                this.getPlayer().getXp().load(b.bb, 247);
            } catch (IOException var4) {
                DebugType.Multiplayer.printException(var4, "Player XP load error", LogSeverity.Error);
            }
        }
    }
