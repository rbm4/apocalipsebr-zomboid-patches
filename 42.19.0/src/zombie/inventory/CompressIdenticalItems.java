// Patched CompressIdenticalItems.java - ApocBR NullCraft patch (Build 42.19)
//
// Change: added null guard to save(ByteBuffer, InventoryItem).
//
...
// the server throws NPE in save(ByteBuffer, InventoryItem) during chunk
// serialization:
...
//
// The patch adds: if (item == null) return;
// at the top of save(ByteBuffer, InventoryItem), silently skipping null items
// instead of crashing and corrupting the save.
//
// Original: zombie.inventory.CompressIdenticalItems (Build 42.19)
...
public final class CompressIdenticalItems {
    ...
    public static void save(ByteBuffer output, InventoryItem item) throws IOException {
        // PATCHED (ApocBR 42.19 NullCraft): null guard to prevent NPE during DryingCraftLogic saves.
        // When DryingCraftLogic references an item that has become null (despawned, player
        // disconnected), the original code threw NPE crashing the chunk save and corrupting
        // vehicles.db. We now silently skip null items instead.
        if (item == null) return;
