...
public final class IsoPuddlesCompute {
    ...
    /**
     * Uses shared static vectors and a shared allocation list. Footstep audio can
     * query puddles from parallel animal updates, so one full calculation must own
     * this scratch state at a time.
     */
    public static synchronized float computePuddle(IsoGridSquare square) {
        pool_vector3f.releaseAll(allocated_vector3f);
        allocated_vector3f.clear();
        hdQuality = false;
        if (!Core.getInstance().getUseShaders()) {
            return -0.1F;
        } else if (GameServer.server || Core.getInstance().getPerfPuddlesOnLoad() != 3 && Core.getInstance().getPerfPuddles() != 3) {
    ...
    private static Vector2f CalculatePuddlesUvTexture(IsoGridSquare square, IsoPuddles isoPuddles) {
        ...
        float fragCoordY = -y;
        return temp_vector2f.set((x * wOffset.z + wOffset.x) * 8.0E-4F + square.z * 7.0F, (fragCoordY * wOffset.w + wOffset.y) * 8.0E-4F + square.z * 7.0F);
    }
