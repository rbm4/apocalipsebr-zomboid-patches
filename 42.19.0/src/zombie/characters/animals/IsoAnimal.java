...
public class IsoAnimal extends IsoPlayer implements IAnimalVisual {
                ...
                for (int y = this.getSquare().getY() - 4; y < this.getSquare().getY() + 4; y++) {
                    ...
                    if (sq != null) {
                        ...
                        for (int i = 0; i < animals.size(); i++) {
                            ...
                            if (animalTest != null && animalTest != this
                                    && animalTest.getAnimalID() == this.getAnimalID()) {
                                DebugType.Animal
                                        .println("Possible chickenpocalypse, deleting the newly created animal");
                                this.delete();
                                return true;
                            }
                        }
    ...
    public void update() {
        if (this.isOnHook()) {
            ...
            if (this.getVariableBoolean("bPathfind")
                    && this.getStateMachine().getCurrent() != AnimalPathFindState.instance()) {
    ...
    private void updateInternal() {
        if (!this.fromMeta) {
            if (this.behavior != null && this.data != null) {
                // Capture volatile fields as locals for thread-safe reads
                // vehicle4testCollision is set from the main thread (player interaction) -
                // reading it on a worker thread requires a local snapshot to avoid torn reads.
                BaseVehicle localVehicle4Test = this.vehicle4testCollision;

                if (GameClient.client) {
                    this.width = this.adef.collisionSize * this.getAnimalSize();
                    this.separate();
                    this.reattachToTree();
                    if (localVehicle4Test != null && localVehicle4Test.updateNetworkHitByVehicle(this)) {
                        super.update();
                        this.vehicle4testCollision = null;
                    } else {
                        super.update();
                    ...
                    this.timeSinceRespondToSound = this.timeSinceRespondToSound
                    ...
                    if (localVehicle4Test != null) {
                        this.setVehicleCollision(this.testCollideWithVehicles(localVehicle4Test, null));
                        this.vehicle4testCollision = null;
                    }
    ...
    private void reattachBackToMom() {
        if (this.attachBackToMother > 0 && this.getVehicle() != null) {
            ArrayList<IsoAnimal> vehicleAnimals = this.getVehicle().getAnimals();
            if (vehicleAnimals != null) {
                // Snapshot: vehicle animals list may be modified on main thread
                ArrayList<IsoAnimal> vehicleAnimalsSnapshot = new ArrayList<>(vehicleAnimals);
                for (int i = 0; i < vehicleAnimalsSnapshot.size(); i++) {
                    IsoAnimal mom = vehicleAnimalsSnapshot.get(i);
                    if (mom != null && mom.getAnimalID() == this.attachBackToMother) {
                        this.setMother(mom);
                        break;
                    }
                }
        ...
        if (this.isExistInTheWorld()) {
            if (this.attachBackToMother == 0 && (this.mother == null || !this.mother.isExistInTheWorld())
                    && this.motherId > 0) {
            ...
            if (this.attachBackToMother > 0 && this.getVehicle() == null) {
                if (this.attachBackToMotherTimer < 50.0F) {
                    this.attachBackToMotherTimer = this.attachBackToMotherTimer
                ...
                for (int k = 0; k < this.connectedDZone.size(); k++) {
                    DesignationZoneAnimal dZone = this.connectedDZone.get(k);
                    if (dZone != null) {
                        ArrayList<IsoAnimal> dZoneAnimals = dZone.getAnimals();
                        if (dZoneAnimals != null) {
                            ArrayList<IsoAnimal> dZoneAnimalsSnapshot = new ArrayList<>(dZoneAnimals);
                            if (this.findMotherAndAttach(dZoneAnimalsSnapshot)) {
                                return;
                            }
                        }
                    }
                ...
                if (cell != null) {
                    List<IsoAnimal> cellAnimals = cell.getAnimals();
                    if (cellAnimals != null) {
                        ArrayList<IsoAnimal> cellAnimalsSnapshot = new ArrayList<>(cellAnimals);
                        this.findMotherAndAttach(cellAnimalsSnapshot);
                    }
                }
            }
        }
    ...
    private boolean findMotherAndAttach(List<IsoAnimal> animals) {
        ...
        for (int i = 0; i < animals.size(); i++) {
            ...
            if (animal != null && animal.animalId == this.attachBackToMother) {
    ...
    public void updateStress() {
        if (!this.isWild()) {
            ...
            if (this.adef.stressUnderRain && RainManager.isRaining() && this.getSquare() != null
    ...
    private void checkTreeExists() {
        ...
        if (tree != null) {
            ...
            IsoGridSquare treeSq = cell
                    .getGridSquare(this.getData().getAttachedTreeX(), this.getData().getAttachedTreeY(),
    ...
    private void reattachToTree() {
        if (this.attachBackToTreeX != Integer.MAX_VALUE && this.attachBackToTreeY != Integer.MAX_VALUE
                && this.getSquare() != null) {
            IsoGridSquare treeSq = this.getSquare().getCell().getGridSquare(this.attachBackToTreeX,
                    this.attachBackToTreeY, this.getSquare().getZ());
            if (treeSq == null) {
                return;
            }
    ...
    public void respondToSound() {
        ...
        if (sound != null && !(attract <= 0.0F) && !(sound.source instanceof IsoRadio)
                && !(sound.source instanceof IsoTelevision)) {
            float dist = IsoUtils.DistanceTo(this.getX(), this.getY(), this.getZ() * 3.0F, sound.x, sound.y,
                    sound.z * 3);
            ...
            if (sound.source instanceof BaseVehicle) {
                radiusBonus *= 10.0F;
            }

            if (this.isWild() || !(sound.source instanceof BaseVehicle)) {
                if (!(dist > sound.radius * radiusBonus)) {
                    if (sound.source instanceof IsoPlayer player && player.callOut
                            && this.getPlayerAcceptance(player) > 40.0F) {
                        ...
                        this.changeStress(sound.radius / 20.0F);
    ...
    public void HitByAnimal(IsoAnimal animal, boolean bIgnoreDamage) {
        ...
        if (!bIgnoreDamage) {
            ...
            if (this.getSquare() != null) {
                IsoGridSquare sq = IsoWorld.instance.currentCell.getGridSquare(this.getSquare().x, this.getSquare().y,
                        this.getSquare().getZ());
                if (sq != null) {
                    for (int i = 0; i < 10; i++) {
                        sq.getChunk().addBloodSplat(sq.x + Rand.Next(-0.8F, 0.8F), sq.y + Rand.Next(-0.8F, 0.8F), sq.z,
                                Rand.Next(8));
                    }
                }
            }
    ...
    private void doDebugString() {
        ...
        if (character != null && character.isAnimalExtraValuesCheat() && !this.isOnHook()) {
            if (this.legsSprite != null && this.legsSprite.modelSlot != null
                    && this.legsSprite.modelSlot.model != null) {
        ...
        if (!this.isOnHook() || this.getHook() != null) {
            if (this.getHook() != null) {
                ...
                if (currentSquare != null) {
                    ...
                    if (PerformanceSettings.fboRenderChunk) {
                        FBORenderShadows.getInstance().addShadow(x, y, z, forward, w, fm, bm, lightInfo.r, lightInfo.g,
                                lightInfo.b, alpha, true);
                    } else {
    ...
    public void hitConsequences(HandWeapon weapon, IsoGameCharacter wielder, boolean bIgnoreDamage, float damage,
        ...
        if (this.isWild() && !bIgnoreDamage) {
            ...
            float dropDeadTimer = Rand.Next(this.getAdef().getWildFleeTimeUntilDeadTimer() / 2.0F,
                    this.getAdef().getWildFleeTimeUntilDeadTimer());
        ...
        if (GameServer.server && (wielder instanceof IsoPlayer || wielder instanceof IsoZombie)
                && this.adef.attackBack) {
    ...
    private void initTexture() {
        ...
        } else if (!StringUtils.isNullOrEmpty(this.adef.textureSkinned)
                && ((KahluaTableImpl) this.getModData()).rawgetBool("skinned")) {
            this.getAnimalVisual().setSkinTextureName(this.adef.textureSkinned);
        } else {
            if (!StringUtils.isNullOrEmpty(this.getData().currentStage.nextStage)
                    && !StringUtils.isNullOrEmpty(this.getBreed().textureBaby)) {
                this.getAnimalVisual().setSkinTextureName(this.getBreed().textureBaby);
            } else if (this.getDescriptor().isFemale()) {
                this.getAnimalVisual()
                        .setSkinTextureName(this.getBreed().texture.get(Rand.Next(0, this.getBreed().texture.size())));
            } else {
    ...
    public boolean canGoThere(IsoGridSquare sq) {
        if (sq.isBlockedTo(this.getCurrentSquare())) {
            ...
            if (this.getData().getAttachedPlayer() != null
                    && sq.DistTo(this.getData().getAttachedPlayer().getCurrentSquare()) > 15.0F) {
                return false;
            } else {
                if (this.getData().getAttachedTree() != null) {
    ...
    public IsoAnimal addBaby() {
        ...
        if (this.adef.babyType == null) {
            ...
            IsoAnimal baby = new IsoAnimal(
                    this.getCell(), this.getXi(), this.getYi(), this.getZi(), this.adef.babyType,
                    babyDef.getBreedByName(this.getBreed().getName()));
            ...
            if (this.adef.isInsideHutchTime(hour)) {
                ...
                if (Rand.NextBool(baseChance)) {
                    if (this.hutch != null && !this.hutch.isDoorClosed()) {
                        ...
                        IsoGridSquare sq = IsoWorld.instance.currentCell
                                .getGridSquare(
                        ...
                        if (sq != null) {
                            for (int i = 0; i < 20; i++) {
                                sq.getChunk().addBloodSplat(sq.x + Rand.Next(-0.8F, 0.8F),
                                        sq.y + Rand.Next(-0.8F, 0.8F), this.hutch.getZ(), Rand.Next(8));
                            }
                        }
                    } else {
                        int nbr = 20;
    ...
    public boolean shearAnimal(IsoGameCharacter chr, InventoryItem shear) {
        if (!(this.getData().woolQty < 1.0F) && !shear.isBroken()) {
            if (shear instanceof DrainableComboItem && shear.getCurrentUsesFloat() <= 0.0F) {
                ...
                if (chr.getPerkLevel(PerkFactory.Perks.Husbandry) <= 5
                        && Rand.NextBool(chr.getPerkLevel(PerkFactory.Perks.Husbandry) + 3)) {
                    this.changeStress(
                            Rand.Next(6.0F - chr.getPerkLevel(PerkFactory.Perks.Husbandry) * 2.0F,
                                    (float) (6 - chr.getPerkLevel(PerkFactory.Perks.Husbandry) * 6))
                                    / 8.0F);
                }
    ...
    public InventoryItem addDebugBucketOfMilk(IsoGameCharacter chr) {
        ...
        newBucket.getFluidContainer().addFluid(this.getBreed().getMilkType(),
                newBucket.getFluidContainer().getCapacity());
        return newBucket;
    }
    ...
    public InventoryItem milkAnimal(IsoGameCharacter chr, InventoryItem bucket) {
        ...
        if (this.data.milkQty < minQty) {
            return null;
        } else if (!bucket.getFluidContainer().isFull() && !(minQty <= 0.0F)) {
            this.getData().updateLastTimeMilked();
            ...
            if (this.milkRemoved >= 50) {
                this.milkRemoved = 0;
                this.getData().maxMilkActual = Math.min(
                        this.getData().maxMilkActual + this.getData().getMaxMilk() * 0.001F,
                        this.getData().getMaxMilk() * 1.3F);
            }
    ...
    public boolean addEgg(boolean meta) {
        if (this.geneticDisorder.contains("noeggs")) {
            ...
            if (eggCont != null && eggCont.getItemContainer() != null
                    && eggCont.getItemContainer().hasRoomFor(null, egg)) {
                ...
                if (foodCont != null && foodCont.getItemContainer() != null
                        && foodCont.getItemContainer().hasRoomFor(null, egg)) {
                    foodCont.getItemContainer().addItem(egg);
                    return true;
                } else {
                    InventoryItem obj = vehicle.getSquare().AddWorldInventoryItem(egg, Rand.Next(0.0F, 0.5F),
            ...
            if (sq != null) {
                InventoryItem obj = this.getSquare().AddWorldInventoryItem(egg, Rand.Next(0.0F, 0.5F),
                        Rand.Next(0.0F, 0.5F), 0.0F);
                IsoWorld.instance.getCell().addToProcessItems(obj);
                IsoWorld.instance.getCell().addToProcessItems(egg);
            }
    ...
    public void randomizeAge() {
        this.setAgeDebug(Rand.Next(this.adef.minAgeForBaby,
                this.adef.minAgeForBaby + this.getData().currentStage.getAgeToGrow(this) - 5));
    }
            ...
            if (list != null && !list.isEmpty()) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (stageCheck.stage.equals(this.getAnimalType())
                            && !StringUtils.isNullOrEmpty(stageCheck.nextStage)) {
                        stage = stageCheck;
                        break;
                    }
                }
    ...
    public boolean isAnimalAttacking() {
        return GameClient.client ? this.isAttackingOnClient
                : this.atkTarget != null || this.adef.canThump && this.thumpTarget != null;
    }
    ...
    public AnimalAllele getUsedGene(String name) {
        ...
        if (geneSet == null) {
            DebugType.Animal
                    .debugln(name + " wasn't found in the full genome of the animal (" + this.getFullName() + ")");
            return null;
        } else {
    ...
    public boolean canDoAction() {
        return !this.isAnimalAttacking() && !this.isAnimalSitting() && !this.getBehavior().blockMovement
                && !this.isAnimalMoving();
    }
            ...
            if (this.CanSee(chr)) {
                if (this.getPossibleLuringItems(chr).contains(item)) {
                    ...
                    if (Rand.Next(100) > acceptance) {
                        ...
                        DebugType.DetailedInfo.trace("Animal id=%d lured by player \"%s\"", this.getOnlineID(),
                                chr.getUsername());
    ...
    public void pathToTrough(IsoFeedingTrough trough) {
        if (trough != null) {
            IsoGridSquare sq1 = IsoWorld.instance.currentCell.getGridSquare((double) trough.getX(),
                    (double) (trough.getY() - 1.0F), (double) trough.getZ());
            IsoGridSquare sq2 = IsoWorld.instance.currentCell.getGridSquare((double) trough.getX(),
                    (double) (trough.getY() + 1.0F), (double) trough.getZ());
            IsoGridSquare sq3 = IsoWorld.instance.currentCell.getGridSquare((double) (trough.getX() - 1.0F),
                    (double) trough.getY(), (double) trough.getZ());
            IsoGridSquare sq4 = IsoWorld.instance.currentCell.getGridSquare((double) (trough.getX() + 1.0F),
                    (double) trough.getY(), (double) trough.getZ());
            if (sq1 != null && sq2 != null && sq3 != null && sq4 != null) {
                ...
                if (!trough.north) {
                    sq3 = sq1;
                    sq4 = sq2;
                    sq1 = IsoWorld.instance.currentCell.getGridSquare((double) (trough.getX() + 1.0F),
                            (double) trough.getY(), (double) trough.getZ());
                    sq2 = IsoWorld.instance.currentCell.getGridSquare((double) (trough.getX() - 1.0F),
                            (double) trough.getY(), (double) trough.getZ());
                }
    ...
    public boolean animalShouldThump() {
        if (!this.adef.canThump) {
            ...
            if (!(this.getStats().get(CharacterStat.THIRST) >= 0.9F)
                    && !(this.getStats().get(CharacterStat.HUNGER) >= 0.9F)) {
    ...
    public boolean isInMatingSeason() {
        if (SandboxOptions.instance.animalMatingSeason.getValue() && this.haveMatingSeason()) {
            ...
            return start < end ? currentMonth >= start && currentMonth < end
                    : currentMonth < end || currentMonth >= start;
        } else {
            return true;
        }
    }
    ...
    public void onPlayBreedSoundEvent(String id) {
        if (!"run".equalsIgnoreCase(id) && !"walk".equalsIgnoreCase(id) && !"walkFront".equalsIgnoreCase(id)
                && !"walkBack".equalsIgnoreCase(id)) {
    ...
    private void chooseIdleSound() {
        ...
        if (this.isDead()) {
            ...
            } else if (this.getStress() > 50.0F && (absStressed != null && ass.isPlaying(absStressed.soundName)
                    || Rand.Next(100) < this.getStress())) {
                desiredSound = "stressed";
            }

            if ("idle".equalsIgnoreCase(desiredSound) && this.data.getBreed().isSoundDefined("idle_walk")
                    && this.isAnimalMoving()) {
                ...
                if (abs == null) {
                    ...
                    if (ass.isPlayingDesiredSound() && abs.isIntervalValid()) {
                        ...
                        if (expireTime < System.currentTimeMillis()) {
                            ass.setIntervalExpireTime(abs.soundName,
                                    System.currentTimeMillis() + Rand.Next(abs.intervalMin, abs.intervalMax) * 1000L);
                        }
                    }
    ...
    public boolean canBePicked(IsoGameCharacter chr) {
        return !chr.isUnlimitedCarry() && !chr.isGodMod()
                ? this.adef.canBePicked
                        && this.getData().getWeight() < 40 + chr.getPerkLevel(PerkFactory.Perks.Strength) * 7
                : true;
    }
    ...
    public void debugForceEgg() {
        ...
        this.getData().eggTime = Long.valueOf(GameTime.instance.getCalender().getTimeInMillis() / 1000L).intValue()
                - 10;
    ...
    public void alertOtherAnimals(IsoMovingObject chr, boolean alert) {
        if (this.getSquare() == null || this.getSquare().getCell() == null) {
            return;
        }
        IsoCell cell = this.getSquare().getCell();
        for (int x = (int) this.getX() - 5; x < (int) this.getX() + 5; x++) {
            for (int y = (int) this.getY() - 5; y < (int) this.getY() + 5; y++) {
                IsoGridSquare sq = cell.getGridSquare((double) x, (double) y, (double) this.getZ());
                if (sq == null) {
                    continue;
                }
                ArrayList<IsoAnimal> animals = sq.getAnimals();
                ...
                // Snapshot to avoid concurrent modification during parallel execution
                ArrayList<IsoAnimal> animalsSnapshot = new ArrayList<>(animals);
                for (int i = 0; i < animalsSnapshot.size(); i++) {
                    IsoAnimal animal = animalsSnapshot.get(i);
    ...
    public void debugForceSit() {
        if (!this.isAnimalSitting()) {
            this.behavior.sitInTime = Long.valueOf(GameTime.instance.getCalender().getTimeInMillis() / 1000L).intValue()
                    - 10;
            this.behavior.sitOutTime = this.behavior.sitInTime + 36000;
        } else {
            this.behavior.sitOutTime = Long.valueOf(GameTime.instance.getCalender().getTimeInMillis() / 1000L)
                    .intValue() - 10;
    ...
    public float getBloodQuantity() {
        float weightPerc = (this.getData().getWeight() - this.adef.minWeight)
                / (this.adef.maxWeight - this.getData().getWeight());
        return (this.adef.minBlood + (this.adef.maxBlood - this.adef.minBlood) * weightPerc) / 100.0F;
    }

    public int getFeatherNumber() {
        return !this.isBaby() && !StringUtils.isNullOrEmpty(this.getBreed().featherItem)
                && this.getBreed().maxFeather > 0
    ...
    public void reattachBackToHook() {
        if (this.attachBackToHookX != 0 || this.attachBackToHookY != 0) {
            if (this.getSquare() != null) {
                IsoGridSquare hookSq = this.getSquare().getCell().getGridSquare(this.attachBackToHookX,
                        this.attachBackToHookY, this.attachBackToHookZ);
    ...
    private void ensureCorrectSkin() {
        if (!StringUtils.isNullOrEmpty(AnimalDefinitions.getDef(this.getAnimalType()).textureSkinned)
                && ((KahluaTableImpl) this.getModData()).rawgetBool("skinned")
                && !this.getAnimalVisual().getSkinTexture()
                        .equalsIgnoreCase(AnimalDefinitions.getDef(this.getAnimalType()).textureSkinned)) {
    ...
    public void updateLOS() {
        ...
        // when the main thread adds/removes objects during parallel execution.
        // One-frame staleness is acceptable for LOS updates.
        ArrayList<IsoMovingObject> objectListSnapshot = new ArrayList<>(cell.getObjectList());

        for (IsoMovingObject movingObject : objectListSnapshot) {
