...
public final class VirtualAnimal {
    ...
    private long apocBrNextUpdateFrame;
    private long apocBrNextTrackFrame;
    ...
    void update() {
        if (MigrationGroupDefinitions.getMigrationDefs().get(this.migrationGroup) != null) {
            if (this.state == null) {
                this.setState(new VirtualAnimalState.StateFollow(this));
    ...
    public String apocBrGetStateName() {
        if (this.state == null) {
            return "Uninitialized";
        } else if (this.state instanceof VirtualAnimalState.StateFollow) {
            return "Follow";
        } else if (this.state instanceof VirtualAnimalState.StateSleep) {
            return "Sleep";
        } else if (this.state instanceof VirtualAnimalState.StateEat) {
            return "Eat";
        } else if (this.state instanceof VirtualAnimalState.StateMoveToSleep) {
            return "MoveToSleep";
        } else if (this.state instanceof VirtualAnimalState.StateMoveToEat) {
            return "MoveToEat";
        } else if (this.state instanceof VirtualAnimalState.StateMoveFromSleep) {
            return "MoveFromSleep";
        } else if (this.state instanceof VirtualAnimalState.StateMoveFromEat) {
            return "MoveFromEat";
        } else {
    ...
    public int apocBrGetUpdateIntervalFrames() {
        if (this.state == null) {
            return 1;
        } else if (this.state instanceof VirtualAnimalState.StateSleep) {
            return 16;
    ...
    public boolean apocBrShouldUpdate(long frameCounter) {
        int interval = this.apocBrGetUpdateIntervalFrames();
        if (interval <= 1) {
            return true;
        } else if (frameCounter >= this.apocBrNextUpdateFrame) {
            this.apocBrNextUpdateFrame = frameCounter + (long)interval;
            return true;
        } else {
            return false;
        }
    ...
    public boolean apocBrShouldAddTrack(long frameCounter, int intervalFrames) {
        if (intervalFrames <= 1) {
            return true;
        } else if (frameCounter >= this.apocBrNextTrackFrame) {
            this.apocBrNextTrackFrame = frameCounter + (long)intervalFrames;
            return true;
        } else {
            return false;
        }
    ...
    public int apocBrGetTrackIntervalFrames() {
        if (this.state instanceof VirtualAnimalState.StateFollow) {
            return 8;
        } else if (this.state instanceof VirtualAnimalState.StateMoveToEat
                || this.state instanceof VirtualAnimalState.StateMoveToSleep
                || this.state instanceof VirtualAnimalState.StateMoveFromEat
                || this.state instanceof VirtualAnimalState.StateMoveFromSleep) {
            return 6;
        } else if (this.state instanceof VirtualAnimalState.StateEat || this.state instanceof VirtualAnimalState.StateSleep) {
            return 16;
        } else {
            return 8;
        }
