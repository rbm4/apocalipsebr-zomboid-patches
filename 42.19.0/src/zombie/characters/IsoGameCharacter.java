...
public abstract class IsoGameCharacter
    ...
    private static final float SNEAK_LIMP_INJURY_THRESHOLD = 0.1F;
    ...
    public static boolean canDropCorpseInto(IsoGameCharacter chr, ItemContainer container) {
        if (chr == null) {
            ...
            return container == null ? false : container.canHumanCorpseFit();
        }
    }
        ...
        if (this.current != null) {
            ...
            if (!this.isDead() || this.current != null && this.current.getMovingObjects().contains(this)) {
                ...
                if (this.isZombie() && VirtualZombieManager.instance.isReused((IsoZombie)this)) {
                    ...
                    if (this instanceof IsoPlayer && !(this instanceof IsoAnimal)) {
                        if (((IsoPlayer)this).getRole().hasCapability(Capability.ToggleWriteRoleNameAbove)) {
                            this.setShowAdminTag(((IsoPlayer)this).calculateShowAdminTag());
                        } else {
    ...
    private float calculateSneakLimpSpeedScale() {
    ...
    protected void calculateWalkSpeed() {
        if (this instanceof IsoPlayer player) {
            if (GameClient.client) {
                boolean runOrSprint = this.running || this.sprinting;
                this.setVariable("WalkSpeed", runOrSprint ? player.networkAi.runSpeed : player.networkAi.walkSpeed);
                float sneakLimpSpeedScale = 1.0F;
        ...
        if (GameServer.server && this instanceof IsoPlayer player) {
            player.networkAi.walkSpeed = walkSpeed;
            player.networkAi.runSpeed = runSpeed;
        }
    }
    ...
    public float onHitByVehicle(BaseVehicle vehicle, float impactSpeed, Vector2 hitDir, Vector2 impactPosOnVehicle) {
        ...
        if (!CombatManager.checkPVP(vehicle, this)) {
            return 0.0F;
        } else {
    ...
    public float onHitByVehicleApplyDamage(BaseVehicle vehicle, float impactSpeed) {
        if (!this.isAlive()) {
            return 0.0F;
        } else if (!CombatManager.checkPVP(vehicle, this)) {
            return 0.0F;
        } else {
            float damage = 0.0F;
    ...
    public void applyDamageFromVehicleHit(BaseVehicle vehicle, float vehicleSpeed, float damage) {
        if (GameServer.server || CombatManager.checkPVP(vehicle, this)) {
    ...
    public boolean causesDamageToVehicleWhenHit(BaseVehicle impactingVehicle) {
        return CombatManager.checkPVP(this, impactingVehicle)
        ...
        if (!canHumanCorpseFit) {
            ...
            if (deadBody == null) {
                ...
                if (inventoryItem == null) {
                    ...
                    return null;
                } else if (!placeInContainer.canItemFit(inventoryItem)) {
                    DebugType.General
                        .error(
    ...
    public void doNetworkHitByVehicle(BaseVehicle hitByVehicle, BaseVehicle.HitVars hitVars) {
        if (GameClient.client || hitVars.isAnimalHit()) {
            if (CombatManager.checkPVP(hitByVehicle, this)) {
                IsoPlayer driver = hitByVehicle.getPVPPlayerDriver();
                if (driver != null) {
                    if (driver.isLocal()) {
                        if (hitVars.anyDamage() || hitVars.isAnimalHit()) {
