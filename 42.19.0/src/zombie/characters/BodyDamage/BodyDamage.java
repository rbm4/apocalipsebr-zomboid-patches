...
public final class BodyDamage {
    ...
    public void Update() {
        if (!(this.parentChar instanceof IsoZombie) && !this.parentChar.isAnimal()) {
            ...
            if (this.parentChar.isGodMod()) {
                // ApocBR Patch: Check if player has admin capabilities before healing
                // Prevents non-admin players from exploiting godmode to heal wounds/infections
                ...
                // Only heal if player is admin
                if (isAdmin) {
                    this.RestoreToFullHealth();
                    ((IsoPlayer)this.parentChar).bleedingLevel = 0;
                }
                // Non-admin: godmode flag is set but healing is disabled
                // Combat immunity still works (CombatManager checks), but no wound healing
            } else {
