...
public final class ZomboidRadio {
        ...
        if (mode == GameMode.Server || mode == GameMode.SinglePlayer) {
            ...
            if (!file.exists()) {
                ...
                try (
                    ...
                    DebugType.General.printException(var24, LogSeverity.Error);
                    return false;
                }
                ...
                try {
                    ...
                    DebugType.General.printException(var20, LogSeverity.Error);
                    var28 = false;
                } finally {
                    result = true;
                }
    ...
    public void UpdateScripts(int hour, int mins) {
        ...
        if (mode == GameMode.Client || mode == GameMode.SinglePlayer) {
            // ApocBR: async chunk/cell retirement can unregister radio devices
            // while the time/radio update is scanning them. Snapshot the live list
            // and skip transient null entries so we do not propagate broken devices.
            ArrayList<WaveSignalDevice> devicesSnapshot = new ArrayList<>(this.devices);
            for (int i = 0; i < devicesSnapshot.size(); i++) {
                WaveSignalDevice device = devicesSnapshot.get(i);
                if (device == null || device.getDeviceData() == null) {
                    continue;
                }

                DeviceData deviceData = device.getDeviceData();
                if (deviceData.getIsTurnedOn() && device.HasPlayerInRange()) {
                    deviceData.TriggerPlayerListening(true);
                }
            }
        }
