...
public final class VehicleManager {
    ...
    public void serverUpdate() {
        Set<BaseVehicle> liveVehicles = IsoWorld.instance.currentCell.getVehicles();
        ArrayList<BaseVehicle> vehicles = new ArrayList<>(liveVehicles.size());
        try {
            vehicles.addAll(liveVehicles);
        ...
        if (this.updateRate.Check()) {
            if (this.updatePassengers.Check()) {
                for (BaseVehicle vehiclex : vehicles) {
                    if (vehiclex == null) {
                        continue;
                    }

                    vehiclex.updateFlags = (short)(vehiclex.updateFlags | 16384);
                }
            }
            ...
            for (BaseVehicle vehiclex : vehicles) {
                ...
                if ((vehiclex.updateFlags & 3056) != 0) {
                    for (int j = 0; j < vehiclex.getPartCount(); j++) {
                        VehiclePart part = vehiclex.getPartByIndex(j);
                        part.updateFlags = 0;
                    }
                }

                vehiclex.updateFlags = 0;
            }
        }
    }
