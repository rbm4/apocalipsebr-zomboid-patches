// Patched ServerChunkLoader.java - ApocBR Async Save CRC fix (Build 42.19)
//
// Change: Replaced shared SaveChunkThread.crc32 with a new CRC32 per call in
// addLoadedJob(). The shared crc32 was not thread-safe: addLoadedJob() is now
// called from WorkerThreads (async save pool) AND the main thread concurrently.
// When two threads interleave crc32.reset() / crc32.update() / crc32.getValue(),
// the CRC value read can be 0, which then gets written into the chunk file
// header. On next load, the sanity check sees save=0 but computes a different
// CRC from the actual data, producing "CRC mismatch save=0 load=..." and a
// SANITY CHECK FAIL crash.
//
// The stale-data race (WorkerThread serializes chunk into ByteBuffer at T1,
...
public class ServerChunkLoader {
    ...
    private class SaveChunkThread extends Thread {
        ...
        // PATCH: Use a new CRC32 per call instead of the shared this.crc32.
        // addLoadedJob() can be called from multiple threads concurrently
        // (WorkerThread from async save pool + main thread for hot-saves).
        // A shared CRC32 is not thread-safe: interleaved reset()/update()/getValue()
        // can produce CRC=0, which corrupts the chunk file header and causes
        // "CRC mismatch save=0 load=..." on next load.
        public void addLoadedJob(IsoChunk chunk) {
            ...
            try {
                chunk.SaveLoadedChunk(reqChunk, new CRC32());
            } catch (Exception var4) {
                DebugType.General.printException(var4, LogSeverity.Error);
                LoggerManager.getLogger("map").write(var4);
