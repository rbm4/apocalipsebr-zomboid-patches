...
public class DefaultShader extends Shader {
    ...
    private float cachedChunkDepth = Float.NaN;
    ...
    public void onCompileSuccess(ShaderProgram sender) {
        ...
        this.chunkDepth = GL20.glGetUniformLocation(shaderID, "chunkDepth");
        this.cachedChunkDepth = Float.NaN;
    }
    ...
    public void setChunkDepth(float depth) {
        if (this.chunkDepth == -1) {
            return;
        }

        if (this.cachedChunkDepth != depth) {
            GL20.glUniform1f(this.chunkDepth, depth);
            this.cachedChunkDepth = depth;
        }
    }
}
