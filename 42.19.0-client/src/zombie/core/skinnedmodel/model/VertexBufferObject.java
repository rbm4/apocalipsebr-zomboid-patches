...
public final class VertexBufferObject {
    ...
    public static void setModelViewProjection(ShaderProgram shaderProgram) {
        if (shaderProgram != null && shaderProgram.isCompiled()) {
            ...
            if (shaderId < 0 || shaderId >= VertexBufferObject.L_setModelViewProjection.shaderStates.length) {
                setModelViewProjectionFallback(shaderProgram);
                return;
            }

            VertexBufferObject.L_setModelViewProjection.ShaderState state = VertexBufferObject.L_setModelViewProjection.shaderStates[shaderId];
            if (state == null) {
                state = new VertexBufferObject.L_setModelViewProjection.ShaderState();
                ShaderProgram.Uniform uMVP = shaderProgram.getUniform("ModelViewProjection", 35676, false);
                state.uniformLocation = uMVP == null ? -1 : uMVP.loc;
                VertexBufferObject.L_setModelViewProjection.shaderStates[shaderId] = state;
            }

            if (state.uniformLocation == -1) {
                return;
            }
            ...
            if (!matrixEquals(modelView, shaderProgram.modelView) || !matrixEquals(projection, shaderProgram.projection)) {
                shaderProgram.modelView.set(modelView);
                shaderProgram.projection.set(projection);
                Matrix4f modelViewProjection = VertexBufferObject.L_setModelViewProjection.MVP;
                modelViewProjection.set(projection);
                modelViewProjection.mul(modelView);
                FloatBuffer floatBuffer = VertexBufferObject.L_setModelViewProjection.FLOAT_BUFFER;
                floatBuffer.clear();
                modelViewProjection.get(floatBuffer);
                floatBuffer.limit(16);
                GL20.glUniformMatrix4fv(state.uniformLocation, false, floatBuffer);
            }
        }
    }

    private static void setModelViewProjectionFallback(ShaderProgram shaderProgram) {
    ...
    private static final class L_setModelViewProjection {
        ...
        static final Matrix4f MVP = new Matrix4f();
        static final FloatBuffer FLOAT_BUFFER = BufferUtils.createFloatBuffer(16);
        static final ShaderState[] shaderStates = new ShaderState[1024];
