...
public final class FBORenderCell {
    ...
    private static final int APOCBR_CHUNK_HOUSEKEEPING_CADENCE = 4;
    private static final int APOCBR_LIGHTING_CHUNK_LEVEL_BUDGET = 3;
    ...
    private boolean runChecks(int playerIndex) {
        ...
        FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
        boolean playerSquareChanged = perPlayerData1.updatePlayerSquare();
        boolean runChunkHousekeeping = playerSquareChanged
            || perPlayerData1.chunkHousekeepingFrame++ % APOCBR_CHUNK_HOUSEKEEPING_CADENCE == 0;
    ...
    private void renderWorldInventoryObjects(IsoGridSquare square, IsoGridSquare renderSquare, boolean bChunkTexture) {
        if (this.shouldRenderSquare(renderSquare)) {
            ...
            this.timSort.doSort(this.tempWorldInventoryObjects.getElements(), (o1, o2) -> {
                IsoWorldInventoryObject w1 = (IsoWorldInventoryObject)o1;
                IsoWorldInventoryObject w2 = (IsoWorldInventoryObject)o2;
                float d1 = w1.xoff * w1.xoff + w1.yoff * w1.yoff;
                float d2 = w2.xoff * w2.xoff + w2.yoff * w2.yoff;
                if (d1 == d2) {
                    return 0;
                } else {
                    return d1 > d2 ? 1 : -1;
    ...
    private void renderAnimatedAttachments(IsoGridSquare square) {
        ...
        for (int i = 0; i < numObjects; i++) {
            IsoObject object = objects[i];
            if (object.getRenderInfo(playerIndex).layer != ObjectRenderLayer.None
                && object.getRenderInfo(playerIndex).layer != ObjectRenderLayer.Translucent
    ...
    private void updateChunkLighting(int playerIndex) {
        if (!DebugOptions.instance.fboRenderChunk.nolighting.getValue()) {
            if (DebugOptions.instance.fboRenderChunk.updateSquareLightInfo.getValue()) {
                FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
                int lightingUpdateCounter = LightingJNI.getUpdateCounter(playerIndex);
                if (perPlayerData1.lightingUpdateCounter != lightingUpdateCounter) {
                    int chunkLevelCount = 0;
                    this.sortedChunks.clear();
                    PZArrayUtil.addAll(this.sortedChunks, perPlayerData1.onScreenChunks);
                    this.timSort.doSort(
                        this.sortedChunks.getElements(),
                        (a, b) -> ((IsoChunk)a).lightingUpdateCounter - ((IsoChunk)b).lightingUpdateCounter,
                        0,
                        this.sortedChunks.size()
                    );
                    ...
                    for (int i = 0; i < this.sortedChunks.size(); i++) {
                        ...
                        for (int z = chunk.minLevel; z <= chunk.maxLevel; z++) {
                            if (this.updateChunkLevelLighting(playerIndex, chunk, z)) {
                                updated = true;
                                chunk.lightingUpdateCounter = lightingUpdateCounter;
                            }
                        }

                        if (updated) {
                            if (++chunkLevelCount >= APOCBR_LIGHTING_CHUNK_LEVEL_BUDGET) {
                                return;
                            }
                        }
                    }

                    perPlayerData1.lightingUpdateCounter = lightingUpdateCounter;
                }
            }
        }
    }
        ...
        if (renderLevels.isOnScreen(level)) {
            ...
            if (squaresObjects.size() + squaresItems.size() != 0) {
                ...
                this.timSort.doSort(sorted.getElements(), (o1, o2) -> {
                    ...
                    IsoGridSquare s1 = (IsoGridSquare)o1;
                    IsoGridSquare s2 = (IsoGridSquare)o2;
                    int i1 = s1.x + s1.y * worldRight;
                    int i2 = s2.x + s2.y * worldRight;
                    return i1 - i2;
                }, 0, sorted.size());
                    ...
                    for (int i = 0; i < perPlayerData1.onScreenChunks.size(); i++) {
                        ...
                        if (z >= chunk.minLevel && z <= chunk.maxLevel) {
                            ...
                            if (renderLevels.isOnScreen(z)) {
                                ...
                                for (int nx = squares.size(); j < nx; j++) {
                                    IsoGridSquare square = squares.get(j);
        ...
        if (GameWindow.states.current == DebugChunkState.instance) {
            ...
            if (DebugChunkState.instance.getBoolean("ObjectAtCursor")) {
                if ("player".equals(DebugChunkState.instance.fromLua1("getObjectAtCursor", "id"))) {
                    ...
                    IsoGridSquare square = IsoWorld.instance.currentCell.getGridSquare((double)gridXf, (double)gridYf, (double)mZ);
                    if (square != null) {
                        float x = player.getX();
                        float y = player.getY();
                        float z = player.getZ();
    ...
    private static final class PerPlayerData {
        ...
        private int chunkHousekeepingFrame;
        private int lastPlayerSquareX = Integer.MIN_VALUE;
        private int lastPlayerSquareY = Integer.MIN_VALUE;
        private int lastPlayerSquareZ = Integer.MIN_VALUE;
        ...
        private boolean updatePlayerSquare() {
            int x = PZMath.fastfloor(IsoCamera.frameState.camCharacterX);
            int y = PZMath.fastfloor(IsoCamera.frameState.camCharacterY);
            int z = PZMath.fastfloor(IsoCamera.frameState.camCharacterZ);
            if (x == this.lastPlayerSquareX && y == this.lastPlayerSquareY && z == this.lastPlayerSquareZ) {
                return false;
            }

            this.lastPlayerSquareX = x;
            this.lastPlayerSquareY = y;
            this.lastPlayerSquareZ = z;
            return true;
        }
        ...
        private void reset() {
            ...
            this.chunkHousekeepingFrame = 0;
            this.lastPlayerSquareX = Integer.MIN_VALUE;
            this.lastPlayerSquareY = Integer.MIN_VALUE;
            this.lastPlayerSquareZ = Integer.MIN_VALUE;
