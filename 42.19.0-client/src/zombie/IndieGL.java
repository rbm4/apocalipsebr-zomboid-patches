...
public final class IndieGL {
    ...
    private static int apocbrLastAlphaFunc = Integer.MIN_VALUE;
    private static float apocbrLastAlphaRef = Float.NaN;
    private static int apocbrLastDepthFunc = Integer.MIN_VALUE;
    private static int apocbrLastDepthMask = -1;
    ...
    public static void glDepthFunc(int a) {
        if (a == apocbrLastDepthFunc) {
            return;
        }

        apocbrLastDepthFunc = a;
        GLState.DepthFunc.set(tempInt.set(a));
    }

    public static void glDepthMask(boolean b) {
        int mask = b ? 1 : 0;
        if (mask == apocbrLastDepthMask) {
            return;
        }

        apocbrLastDepthMask = mask;
    ...
    public static void glAlphaFunc(int a, float b) {
        if (SpriteRenderer.glBlendfuncEnabled) {
            if (a == apocbrLastAlphaFunc && b == apocbrLastAlphaRef) {
                return;
            }

            apocbrLastAlphaFunc = a;
            apocbrLastAlphaRef = b;
