...
public final class BaseVehicle extends IsoMovingObject implements Thumpable, IFMODParameterUpdater, IPositional, VehicleSoundOwner {
    ...
    public static final float DOT_PRODUCT_ATTACH_TRAILER = 0.6F;
    ...
    public UpdateSchedulerSimulationLevel apocBrGetClientSimulationLevel() {
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoAttached, boolean bWallLightingPass, Shader shader) {
        if (this.script != null) {
            ...
            if (bForceSeen || this.square.lighting[playerIndex].bSeen()) {
                ...
                if (this.sprite.hasActiveModel()) {
                    boolean apocbrSkipLights = false;
                    if (this.isAtRest() && !bForceSeen) {
                        IsoGameCharacter cameraCharacter = IsoCamera.getCameraCharacter();
                        apocbrSkipLights = cameraCharacter != null && this.DistToSquared(cameraCharacter) > 900.0F && this.getID() % 4 != 0;
                    }
    ...
    public void renderShadow() {
        if (this.isAtRest()) {
            IsoGameCharacter cameraCharacter = IsoCamera.getCameraCharacter();
            if (cameraCharacter != null && this != cameraCharacter.getVehicle() && this.DistToSquared(cameraCharacter) > 900.0F) {
                return;
            }
        }
        ...
        if (this == vehicleB || this.physics == null || this.constraintTowing != -1) {
            ...
            if (v1 != null && v2 != null) {
                ...
                if (attachA != null && attachA.getCanAttach() != null && !attachA.getCanAttach().contains(attachmentB)) {
                    ...
                    Vector3f vector3f_forward = TL_vector3f_pool.get().alloc();
                    this.getForwardVector(vector3f_forward);
                    Vector3f vector3f_trailer_forward = TL_vector3f_pool.get().alloc();
                    vehicleB.getForwardVector(vector3f_trailer_forward);
                    if (vector3f_forward.dot(vector3f_trailer_forward) < 0.6F) {
                        return false;
                    } else {
