...
public final class IsoZombie extends IsoGameCharacter implements IHumanVisual {
    ...
    public UpdateSchedulerSimulationLevel getMinimumSimulationLevel() {
        ...
        if (this.isIdleOrStaggering()) {
            for (int i = 0; i < IsoPlayer.numPlayers; i++) {
                if (this.isVisibleToPlayer[i]) {
    ...
    protected boolean TestIfSeen(int playerIndex, IsoPlayer player) {
        if ((this.getID() + IsoCamera.frameState.frameCount + playerIndex) % 4 != 0) {
            return this.isVisibleToPlayer[playerIndex];
        }
        return super.TestIfSeen(playerIndex, player);
    }
    ...
    public void setTurnAlertedValues(int soundX, int soundY) {
        ...
        if (degreeOfSound > degreeOfZombie) {
            ...
            int sumUpDegreex = (int)(degreeOfZombie - degreeOfSound);
            if (sumUpDegreex > 10 && sumUpDegreex <= 55) {
                trueDegree = "45L";
            }

            if (sumUpDegreex > 55 && sumUpDegreex <= 100) {
                trueDegree = "90L";
            }

            if (sumUpDegreex > 100 && sumUpDegreex <= 145) {
                trueDegree = "135L";
            }

            if (sumUpDegreex > 145 && sumUpDegreex <= 190) {
                trueDegree = "180L";
            }

            if (sumUpDegreex > 190 && sumUpDegreex < 235) {
                trueDegree = "180R";
            }

            if (sumUpDegreex >= 235 && sumUpDegreex < 280) {
                trueDegree = "135R";
            }

            if (sumUpDegreex >= 280 && sumUpDegreex < 325) {
                trueDegree = "90R";
            }

            if (sumUpDegreex >= 325 || sumUpDegreex < 10) {
                trueDegree = "45R";
            }
        }
    ...
    public void moveUnmodded(float dirX, float dirY) {
        ...
        if (GameClient.client && this.isRemoteZombie()) {
            ...
            float movingScale = Math.abs(dist) < 0.01F
                ? 1.0F
                : 0.5F + IsoUtils.smoothstep(0.5F, 1.5F, IsoUtils.DistanceTo(this.getX(), this.getY(), this.networkAi.targetX, this.networkAi.targetY) / dist);
            dirX *= movingScale;
            dirY *= movingScale;
            ...
            if (!bMoving) {
                ...
                if (feeler != null && this.current != null) {
                    ...
                    if (window != null && window.canClimbThrough(this)) {
                        ...
                        return true;
                    } else if (thumpable == null || !thumpable.isHoppable() || thumpable.isTallHoppable()) {
                    ...
                    } else if (!this.isFacingObject(thumpable, 0.8F)) {
                        return false;
                    } else {
                        IsoDirections dirx = this.getCardinalDirectionTo(thumpable.getSquare(), thumpable.getNorth());
                        this.climbOverFence(dirx);
                        return true;
                    }
                } else {
                    return false;
