...
public final class ComponentContainer {
    ...
    private static final int INITIAL_BUCKET_SLOT_CAPACITY = 16;
    ...
    private int[] bucketSlots = new int[INITIAL_BUCKET_SLOT_CAPACITY];
    ...
    int getBucketSlot(int bucketIndex) {
        return bucketIndex < this.bucketSlots.length ? this.bucketSlots[bucketIndex] : -1;
    }

    void setBucketSlot(int bucketIndex, int slot) {
        if (bucketIndex >= this.bucketSlots.length) {
            int newLength = Math.max(this.bucketSlots.length * 2, bucketIndex + 1);
            this.bucketSlots = Arrays.copyOf(this.bucketSlots, newLength);
        }

        this.bucketSlots[bucketIndex] = slot;
    }
