...
public final class IsoFeedingTrough extends IsoObject {
    ...
    // === ApocBR: trough feed loss fix ==========================================
    // Water and feed are mutually exclusive in a trough, and vanilla resolves the
    // conflict by dropping the whole feed ItemContainer (setContainer(null)) as
    // soon as the FluidContainer holds anything. The only thing protecting
    // player-placed feed is checkWaterFromRain(), which detaches the rain-catching
    // FluidContainer while feed is present - and vanilla only calls it from
    // update().
    //
    // That is a one-frame race in vanilla, because IsoCell.ProcessIsoObject() ran
    // update() on every IsoObject every frame. ApocBR phases IsoObject updates
    // (apocbr.isoObjectUpdateFrameMod, default 8), which widens the race by the
    // same factor and makes the loss routine. It is also permanently open for meta
    // troughs, which never get an update() at all while their chunk is unloaded but
    // are still rained into by FluidContainerUpdateSystem.
    //
    // The rule this enforces is simply "feed wins over rain": rain can never
    // silently destroy feed, whichever path reaches checkContainer() (update(),
    // addToWorld(), or FluidContainer -> onFluidContainerUpdate()). The rain catcher
    // is still detached by checkWaterFromRain() on the next update(), exactly as
    // vanilla intends, and the water collected in the meantime goes with it.
    //
    // Note we deliberately do NOT call checkWaterFromRain() from here:
    // removeFluidContainer() ends up in GameEntity.releaseComponent(), which resets
    // and pools the Component synchronously, and one of our callers is FluidContainer's
    // own update callback - we would be freeing the object under our own call stack.
    //
    // A deliberate player pour still replaces the feed like vanilla; see addWater().
    ...
    public void addWater(FluidType type, float amount) {
        ...
        // ApocBR: checkContainer() now refuses to drop feed for rain (see its comment).
        // A player deliberately filling the trough with water is not rain, so keep the
        // vanilla outcome here and clear the feed explicitly.
