    ...
    public void separate() {
        if (this.isSolidForSeparate()) {
            if (this.isPushableForSeparate()) {
                ...
                IsoPlayer thisPlyr = this instanceof IsoPlayer tp ? tp : null;
                IsoZombie thisZombie = this instanceof IsoZombie tz ? tz : null;
                float maxWeaponRange = thisPlyr != null && thisPlyr.getPrimaryHandItem() instanceof HandWeapon
                    ...
                    : 0.3F;
                boolean stationaryServerZombie = thisZombie != null
                    && GameServer.server
                    && thisZombie.getTarget() == null
                    && !thisZombie.isMoving()
                    && Math.abs(this.getNextX() - this.getX()) < 1.0E-4F
                    && Math.abs(this.getNextY() - this.getY()) < 1.0E-4F;
                if (stationaryServerZombie && this.current.getMovingObjects().size() <= 1) {
                    return;
                }

                for (int i = stationaryServerZombie ? 8 : 0; i <= 8; i++) {
                    IsoGridSquare sq = i == 8 ? this.current : this.current.getSurroundingSquares()[i];
                    if (sq != null) {
                        List<IsoMovingObject> movingObjects = sq.getMovingObjects();
                        if (!movingObjects.isEmpty() && (sq == this.current || !this.current.isBlockedTo(sq))) {
                            int n = 0;

                            for (int size = movingObjects.size(); n < size; n++) {
                                ...
                                if (obj != this && obj.isSolidForSeparate() && !(Math.abs(this.getZ() - obj.getZ()) > 0.3F)) {
                                    ...
                                    float diffLenSq = diff.x * diff.x + diff.y * diff.y;
                                    if (thisChr == null || objChr == null && !(obj instanceof BaseVehicle)) {
                                        if (diffLenSq < twidth * twidth) {
                                    ...
                                    if (objChr != null) {
                                        float maxActingRange = twidth + maxWeaponRange;
                                        if (!(diffLenSq < maxActingRange * maxActingRange)) {
                                            continue;
                                        }
                                        ...
                                        if (!(len >= twidth)) {
                                            IsoPlayer objPlyr = obj instanceof IsoPlayer op ? op : null;
                                            IsoZombie objZombie = obj instanceof IsoZombie oz ? oz : null;
                                            boolean bump = false;
                                            if (thisPlyr != null
