...
public abstract class IsoGameCharacter
    ...
    // ApocBR: these were shared static Vector2 scratch buffers. With parallel chunk loading
    // (ServerChunkLoader.LOAD_WORKERS) several threads run IsoGameCharacter/IsoAnimal load()
    // at once, so interleaved writes could be observed as a (0,0) vector and
    // setForwardDirection() threw "Forward Direction cannot be zero length vector.".
    private static final ThreadLocal<Vector2> tempVector2_1TL = ThreadLocal.withInitial(Vector2::new);
    private static final ThreadLocal<Vector2> tempVector2_2TL = ThreadLocal.withInitial(Vector2::new);
    ...
    // ApocBR: thread-local scratch buffer, see tempVector2_1TL/tempVector2_2TL declaration.
    private static final ThreadLocal<Vector2> tempVector2TL = ThreadLocal.withInitial(Vector2::new);
    ...
    protected void damageWhileInTrees() {
        if (!GameClient.client && !this.isZombie() && !"Tutorial".equals(Core.gameMode)) {
            ...
            if (Rand.NextBool(rand)) {
                ...
                IsoPlayer player = Type.tryCastTo(this, IsoPlayer.class);
                if (Rand.NextBool((int)var6) && (int)this.getBodyPartClothingDefense(part, false, false) < 100) {
    ...
    public void setForwardDirectionFromIsoDirection() {
        Vector2 dir = tempVector2_2TL.get();
        this.getVectorFromDirection(dir);
        this.setForwardDirection(dir);
    }
    ...
    public float processHitDamage(HandWeapon weapon, IsoGameCharacter wielder, float damageSplit, boolean bIgnoreDamage, float modDelta) {
        if (weapon.isExplosive()) {
            ...
            if (wielder instanceof IsoPlayer isoPlayer && !isoPlayer.isDoShove() && !bIgnoreDamage) {
                Vector2 oPos = tempVector2_1TL.get().set(this.getX(), this.getY());
                Vector2 tPos = tempVector2_2TL.get().set(wielder.getX(), wielder.getY());
                oPos.x = oPos.x - tPos.x;
                oPos.y = oPos.y - tPos.y;
                Vector2 dir = this.getVectorFromDirection(tempVector2_2TL.get());
                oPos.normalize();
            ...
            if (sneezeText != null) {
                TextManager.instance
                    .DrawStringCentre(
                        ...
                        (double)((int)sy),
                        sneezeText,
                        (double)this.speakColour.r,
                        (double)this.speakColour.g,
                        (double)this.speakColour.b,
                        (double)this.speakColour.a
                    );
            }
        }
    }
    ...
    public void renderlast() {
        ...
        if (this.hasInitTextObjects && this.isoPlayer != null || this.chatElement.getHasChatToDisplay()) {
            ...
            Vector2 nameCoords = tempVector2_2TL.get();
            if (vehicle == null || !vehicle.getNameCoordForPlayer(this, zoom, nameCoords)) {
                float nameOffY = 128 / (2 / Core.tileScale);
                getNameCoords(renderX, renderY, renderZ, this.offsetX, this.offsetY + nameOffY, zoom, nameCoords);
            }

            float sx = nameCoords.x;
            float sy = nameCoords.y;
                ...
                if (this.ballisticsController != null && weapon.isRanged()) {
                    ...
                    for (int i = 0; i < this.hitInfoList.size(); i++) {
                        ...
                        if (obj != null) {
                            ...
                            TextManager.instance.DrawStringCentre(font, (double)sx4, (double)sy4, String.valueOf(hitInfo.dot), 1.0, 1.0, 1.0, 1.0);
                            TextManager.instance
                                .DrawStringCentre(
                                    font, (double)sx4, (double)(sy4 + TextManager.instance.getFontHeight(font)), hitInfo.chance + "%", 1.0, 1.0, 1.0, 1.0
                                );
                            r = 1.0F;
                            g = 1.0F;
                            float b = 1.0F;
                            ...
                            TextManager.instance
                                .DrawStringCentre(
                                    font,
                                    (double)sx4,
                                    (double)(sy4 + TextManager.instance.getFontHeight(font) * 2),
                                    "DIST: " + dist,
                                    (double)r,
                                    (double)g,
                                    (double)b,
        ...
        if (this == IsoPlayer.getInstance()) {
            ...
            for (int i = 0; i < zombies.size(); i++) {
                ...
                if (this.DistToSquared(obj) < 4.0F) {
                    ...
                    TextManager.instance.DrawStringCentre(font, (double)sx4, (double)(sy4 + fontHgt), "SIDE: " + this.testDotSide(obj), 1.0, 1.0, 1.0, 1.0);
                    ...
                    TextManager.instance
                        .DrawStringCentre(font, (double)sx4, (double)(sy4 + fontHgt * 2), "ANGLE (0-360): " + PZMath.radToDeg(radians), 1.0, 1.0, 1.0, 1.0);
                    radians = (float)Math.acos(this.getDotWithForwardDirection(obj.getX(), obj.getY()));
                    TextManager.instance
                        .DrawStringCentre(font, (double)sx4, (double)(sy4 + fontHgt * 3), "ANGLE (0-180): " + PZMath.radToDeg(radians), 1.0, 1.0, 1.0, 1.0);
                }
            }
        }
    }
    ...
    protected void calculateWalkSpeed() {
        ...
        float runSpeed = walkSpeed - 0.15F;
    ...
    public float Hit(
        ...
        float var10;
        try {
            var10 = this.onHitByVehicle(vehicle, speed, this.getHitDir().set(hitDirX, hitDirY), impactPosOnVehicle);
        } finally {
    ...
    public boolean isObjectBehind(IsoObject obj) {
        Vector2 oPos = tempVector2_1TL.get().set(obj.getX(), obj.getY());
        Vector2 tPos = tempVector2_2TL.get().set(this.getX(), this.getY());
    ...
    public boolean isBehind(IsoGameCharacter chr) {
        Vector2 oPos = tempVector2_1TL.get().set(this.getX(), this.getY());
        Vector2 tPos = tempVector2_2TL.get().set(chr.getX(), chr.getY());
        tPos.x = tPos.x - oPos.x;
        tPos.y = tPos.y - oPos.y;
                ...
                if (Rand.NextBool(4 + this.getPerkLevel(PerkFactory.Perks.Nimble))) {
                    ...
                    if (Rand.Next(100) >= defense) {
                        ...
                        if (bodyPart.scratched()) {
                            ...
                            bodyPart.setScratched(true, true);
                            if (this instanceof IsoPlayer playerx) {
                                if (GameServer.server) {
                                    playerx.getNetworkCharacterAI().syncDamage();
                                    GameServer.sendCharacterSound(this, "PainFromScratch", (byte)2);
                                } else {
                                    playerx.playerVoiceSound("PainFromScratch");
                                }
                            }
                        }
                    }

                    if (this instanceof IsoPlayer playerxx) {
                        playerxx.syncVisuals();
                    }
                }
            }
        }
    ...
    public void readInventory(ByteBufferReader b) {
        try {
            ...
            DebugType.Multiplayer.printException(var8, "ReadInventory error id=" + this.getOnlineID(), LogSeverity.Error);
        }
    }
    ...
    private void renderDebugData() {
        ...
        if (this.getNetworkCharacterAI().getBooleanDebugOptions().prediction.getValue()) {
            ...
            dy += 4.0F;
            if (isoZombie != null) {
                stringDrawer.draw(font, sx, sy + (dy += 14.0F), "Prediction: " + this.getNetworkCharacterAI().predictionType, cx.r, cx.g, cx.b, cx.a);
        ...
        if (this.getNetworkCharacterAI().getBooleanDebugOptions().variables.getValue()) {
            Color cx = Colors.MediumPurple;
            float var20 = dy + 4.0F;
            float var21;
            stringDrawer.draw(font, sx, sy + (var21 = var20 + 14.0F), String.format("FallTime: %.03f", this.getFallTime()), cx.r, cx.g, cx.b, cx.a);
            stringDrawer.draw(font, sx, sy + (dy = var21 + 14.0F), String.format("Reanimate: %.03f", this.getReanimateTimer()), cx.r, cx.g, cx.b, cx.a);
            if (isoPlayer != null && isoAnimal == null) {
                float var22;
                stringDrawer.draw(
                    font, sx, sy + (var22 = dy + 14.0F), String.format("SneakLimpSpeedScale: %s", isoPlayer.getSneakLimpSpeedScale()), cx.r, cx.g, cx.b, cx.a
                );
                float var23;
                stringDrawer.draw(
                    font,
                ...
                float var30;
                stringDrawer.draw(
                    font, sx, sy + (var30 = var29 + 14.0F), String.format("ForwardDirection: %f", isoPlayer.getDirectionAngleRadians()), cx.r, cx.g, cx.b, cx.a
                );
                stringDrawer.draw(
                    font, sx, sy + (dy = var30 + 14.0F), String.format("FishingStage: %s", isoPlayer.getVariableString("FishingStage")), cx.r, cx.g, cx.b, cx.a
                );
            }

            if (isoAnimal != null) {
                float var31;
                stringDrawer.draw(font, sx, sy + (var31 = dy + 14.0F), String.format("Stress:%.02f", isoAnimal.getStress()), cx.r, cx.g, cx.b, cx.a);
                float var32;
                stringDrawer.draw(
                    font, sx, sy + (var32 = var31 + 14.0F), String.format("Milk: %.02f", isoAnimal.getData().getMilkQuantity()), cx.r, cx.g, cx.b, cx.a
                );
                float var33;
                stringDrawer.draw(
                    font,
                ...
                stringDrawer.draw(
                    font, sx, sy + (dy = var33 + 14.0F), String.format("AlertX:%.02f", isoAnimal.getVariableFloat("AlertX", 0.0F)), cx.r, cx.g, cx.b, cx.a
                );
            }

            if (isoZombie != null) {
                if (isoZombie.target instanceof IsoPlayer player) {
                    stringDrawer.draw(font, sx, sy + (dy += 14.0F), "Target: " + player.username, cx.r, cx.g, cx.b, cx.a);
                } else if (isoZombie.target != null) {
                    stringDrawer.draw(font, sx, sy + (dy += 14.0F), "Target: " + isoZombie.target, cx.r, cx.g, cx.b, cx.a);
                } else {
                    float var34;
                    stringDrawer.draw(font, sx, sy + (var34 = dy + 14.0F), "Target x=" + this.getPathTargetX(), cx.r, cx.g, cx.b, cx.a);
                    stringDrawer.draw(font, sx, sy + (dy = var34 + 14.0F), "Target y=" + this.getPathTargetY(), cx.r, cx.g, cx.b, cx.a);
                }
            }
        }

        if (this.getNetworkCharacterAI().getBooleanDebugOptions().state.getValue()) {
            Color cxx = Colors.LightBlue;
            dy += 4.0F;
            String subStateName = "";
            ...
            stringDrawer.draw(
                ...
                sx,
                sy + (var36 = dy + 14.0F),
                String.format("Class state: %s ( %s )", this.getCurrentStateName(), subStateName),
                cxx.r,
                cxx.g,
                cxx.b,
                cxx.a
            ...
            if (this.characterActions != null) {
                stringDrawer.draw(font, sx, sy + (dy += 14.0F), String.format("Actions: %d", this.characterActions.size()), cxx.r, cxx.g, cxx.b, cxx.a);

                for (BaseAction baseAction : this.characterActions) {
                    if (baseAction instanceof LuaTimedActionNew luaTimedActionNew) {
                        stringDrawer.draw(
                            font, sx, sy + (dy += 14.0F), String.format("Action: %s", luaTimedActionNew.getMetaType()), cxx.r, cxx.g, cxx.b, cxx.a
                        );
                    }
                }
            }
            ...
            stringDrawer.draw(font, sx, sy + (dy = dy + 14.0F), String.format("Real state: %s", this.realState), cxx.r, cxx.g, cxx.b, cxx.a);
        }

        if (this.getNetworkCharacterAI().getBooleanDebugOptions().stateVariables.getValue()) {
            Color cxxx = Colors.LightGreen;
            dy += 4.0F;
            float var41;
            stringDrawer.draw(
                font,
            ...
            stringDrawer.draw(
                ...
                sx,
                sy + (var47 = dy + 14.0F),
                String.format("isAiming: %b / %b", this.isAiming(), this.getVariableBoolean("aim")),
                cxxx.r,
                cxxx.g,
                cxxx.b,
                cxxx.a
            ...
            if (isoZombie != null) {
                float var48;
                stringDrawer.draw(
                    font, sx, sy + (var48 = var47 + 14.0F), String.format("bThump: %b", this.getVariableString("bThump")), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
                float var49;
                stringDrawer.draw(
                    font, sx, sy + (var49 = var48 + 14.0F), String.format("ThumpType: %s", this.getVariableString("ThumpType")), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
                float var50;
                stringDrawer.draw(
                    font, sx, sy + (var50 = var49 + 14.0F), String.format("onknees: %b", this.getVariableBoolean("onknees")), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
                float var51;
                stringDrawer.draw(font, sx, sy + (var51 = var50 + 14.0F), String.format("isCanWalk: %b", isoZombie.isCanWalk()), cxxx.r, cxxx.g, cxxx.b, cxxx.a);
                float var52;
                stringDrawer.draw(
                    font, sx, sy + (var52 = var51 + 14.0F), String.format("isCrawling: %b", isoZombie.isCrawling()), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
                stringDrawer.draw(
                    font, sx, sy + (dy = var52 + 14.0F), String.format("isBecomeCrawler: %b", isoZombie.isBecomeCrawler()), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
            } else {
                float var53;
                stringDrawer.draw(
                ...
                stringDrawer.draw(
                    font, sx, sy + (dy = var54 + 14.0F), String.format("bPathfind: %s", this.getVariableBoolean("bPathfind")), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
                if (isoAnimal != null) {
                    float var55;
                    stringDrawer.draw(
                        font, sx, sy + (var55 = dy + 14.0F), String.format("isAlerted: %b", isoAnimal.isAlerted()), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                    );
                    float var56;
                    stringDrawer.draw(
                        font,
                    ...
                    float var57;
                    stringDrawer.draw(font, sx, sy + (var57 = dy + 14.0F), String.format("isGrappling: %b", this.isGrappling()), cxxx.r, cxxx.g, cxxx.b, cxxx.a);
                    float var58;
                    stringDrawer.draw(
                        font, sx, sy + (var58 = var57 + 14.0F), String.format("isDoGrapple: %b", this.isDoGrapple()), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                    );
                    float var59;
                    stringDrawer.draw(
                        font,
                        sx,
                        sy + (var59 = var58 + 14.0F),
                        String.format("isDoContinueGrapple: %b", this.isDoContinueGrapple()),
                        cxxx.r,
                        cxxx.g,
                        cxxx.b,
                        cxxx.a
                    ...
                    stringDrawer.draw(
                        font,
                        sx,
                        sy + (var61 = var60 + 14.0F),
                        String.format("initiateAttack: %b", isoPlayer.isInitiateAttack()),
                        cxxx.r,
                        cxxx.g,
                        cxxx.b,
                        cxxx.a
                    ...
                    stringDrawer.draw(
                        ...
                        String.format("rangedWeapon: %b", this.getVariableBoolean("rangedWeapon")),
                        cxxx.r,
                        cxxx.g,
                        cxxx.b,
                        cxxx.a
                    );
                    stringDrawer.draw(font, sx, sy + (dy = var62 + 14.0F), String.format("bDoShove: %b", isoPlayer.isDoShove()), cxxx.r, cxxx.g, cxxx.b, cxxx.a);
                }
            }
        }
        ...
        if (this.getNetworkCharacterAI().getBooleanDebugOptions().enable.getValue()
            ...
            Color cxxx = Colors.YellowGreen;
            dy += 4.0F;
            if (this.getAdvancedAnimator().getRootLayer() != null) {
                float var64;
                stringDrawer.draw(
                    font, sx, sy + (var64 = dy + 14.0F), "State: " + this.getAdvancedAnimator().getCurrentStateName(), cxxx.r, cxxx.g, cxxx.b, cxxx.a
                );
                AnimationPlayer animationPlayer = this.getAnimationPlayer();
                if (animationPlayer != null) {
                    for (AnimationTrack track : animationPlayer.getMultiTrack().getTracks()) {
                        stringDrawer.draw(font, sx, sy + (var64 += 14.0F), "Clip: " + track.currentClip.name, cxxx.r, cxxx.g, cxxx.b, cxxx.a);
                    }
                }
            }
        }
    ...
    public AimingMode getAimingMode() {
        ...
        } else if (this.isMeleeWeaponEquipped()) {
            return this.hasTarget ? AimingMode.MELEE_TARGET_FOUND : AimingMode.MELEE;
        } else if (this.isRangedWeaponEquipped()) {
    ...
    private void updateMovementStatistics() {
        if (this.isoPlayer != null) {
            Vector2 lastPos = tempVector2_1TL.get().set(this.getLastX(), this.getLastY());
            Vector2 currentPos = tempVector2_2TL.get().set(this.getX(), this.getY());
            float distanceTraveled = lastPos.distanceTo(currentPos);
            if (this.isSprinting()) {
    ...
    protected void createFallingItem(InventoryItem item) {
        ...
        float z = this.getZ();
        if (GameServer.server) {
            IsoGridSquare square = this.getCell().getGridSquare((double)x, (double)y, (double)z);
    ...
    public static class TorchInfo {
        ...
        public IsoGameCharacter.TorchInfo set(IsoPlayer p, InventoryItem item) {
            ...
            this.g = 0.8235294F;
            this.b = 0.7058824F;
            Vector2 lookVector = p.getLookVector(IsoGameCharacter.tempVector2TL.get());
    ...
    public class XP {
            ...
            if (!IsoGameCharacter.this.isAsleep()) {
                ...
                if (perk.getType() != PerkFactory.Perks.Fitness || !(this.chr instanceof IsoPlayer isoPlayer && !isoPlayer.getNutrition().canAddFitnessXp())) {
                    ...
                    if (!(amount >= 0.0F) || !(oldXP >= maxXP)) {
                        ...
                        if (this.chr instanceof IsoPlayer playerx && perk == PerkFactory.Perks.Fitness) {
                            playerx.getStats().set(CharacterStat.FITNESS, playerx.getPerkLevel(perk) / 5.0F - 1.0F);
                        }
        ...
        public void load(ByteBuffer input, int worldVersion) throws IOException {
            ...
            for (int nx = 0; nx < perkListSize; nx++) {
            ...
            for (int nxx = 0; nxx < xpMapMultiplierSize; nxx++) {
