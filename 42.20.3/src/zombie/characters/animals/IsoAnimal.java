...
public class IsoAnimal extends IsoPlayer implements IAnimalVisual {
    ...
    // ApocBR: these were shared static scratch objects. With parallel chunk loading
    // (ServerChunkLoader.LOAD_WORKERS) several threads touch animals concurrently, so any
    // shared scratch can be read half-written by another thread. tempVector2 is kept as a
    // field because vanilla classes (AnimalAlertedState) reference it directly, but all uses
    // inside this class now go through the thread-local buffer.
    private static final ThreadLocal<Vector3f> tempVector3fTL = ThreadLocal.withInitial(Vector3f::new);
    private static final ThreadLocal<Vector3> tempVector3TL = ThreadLocal.withInitial(Vector3::new);
    ...
    // ApocBR: thread-local scratch buffer, see tempVector3fTL declaration.
    private static final ThreadLocal<Position3D> L_renderCustomNameTL = ThreadLocal.withInitial(Position3D::new);
    ...
    public float Hit(
        ...
        Vector2 hitDir = tempVector2TL.get().set(hitDirX, hitDirY);
        return this.Hit(vehicle, speed, isHitFromBehind, hitDir);
    }
        ...
        if (sound != null && !(attract <= 0.0F) && !(sound.source instanceof IsoRadio) && !(sound.source instanceof IsoTelevision)) {
            if (this.isWild() || !sound.sourceIsVehicle()) {
                if (sound.source instanceof IsoPlayer player && player.callOut && this.getPlayerAcceptance(player) >= 40.0F) {
                    ...
                    Vector2 fleeVector = tempVector2TL.get().set(this.getX() - sound.x, this.getY() - sound.y);
                    ...
                    fleeVector.setLength(fleeDistance - fleeVector.getLength());
                    int goX = fleeVector.floorX();
                    int goY = fleeVector.floorY();
    ...
    public void drawRope(IsoGameCharacter chr) {
        ...
        Vector3 boneCoords = tempVector3TL.get();
        Model.boneToWorldCoords(this, bone, boneCoords);
        float r = 0.5F;
        float g = 0.37F;
        float b = 0.3F;
        ...
        float sx0 = IsoUtils.XToScreenExact(boneCoords.x, boneCoords.y, boneCoords.z, 0);
        sx0 -= this.getAnimalSize();
        float sy0 = IsoUtils.YToScreenExact(boneCoords.x, boneCoords.y, boneCoords.z, 0);
        sy0 -= this.getAnimalSize();
        bone = chr.getAnimationPlayer().getSkinningBoneIndex("Bip01_R_Finger0", -1);
        Model.boneToWorldCoords(chr, bone, boneCoords);
        float sx1 = IsoUtils.XToScreenExact(boneCoords.x, boneCoords.y, boneCoords.z, 0);
        float sy1 = IsoUtils.YToScreenExact(boneCoords.x, boneCoords.y, boneCoords.z, 0);
        ...
        if (player != null && !player.isDead()) {
            if (player.isSeeDesignationZone()) {
                if (!StringUtils.isNullOrWhitespace(this.getCustomName())) {
                    ...
                    Position3D p3d = this.getAttachmentWorldPos("head", L_renderCustomNameTL.get());
                    float renderZ;
                    if (p3d == null) {
                        renderZ = 0.5F;
                    } else {
    ...
    public void renderShadow(float x, float y, float z) {
        if (!this.isOnHook() || this.getHook() != null) {
            if (this.getHook() != null) {
                int playerIndex = IsoCamera.frameState.playerIndex;
                Vector3f forward = tempVector3fTL.get();
        ...
        if (!(this.getData().woolQty < 1.0F) && !shear.isBroken()) {
            if (shear instanceof DrainableComboItem && shear.getCurrentUsesFloat() <= 0.0F) {
                ...
                if (chr.getPerkLevel(PerkFactory.Perks.Husbandry) <= 5 && Rand.NextBool(chr.getPerkLevel(PerkFactory.Perks.Husbandry) + 3)) {
                    this.changeStress(
                        Rand.Next(6.0F - chr.getPerkLevel(PerkFactory.Perks.Husbandry) * 2.0F, (float)(6 - chr.getPerkLevel(PerkFactory.Perks.Husbandry) * 6))
                            / 8.0F
                    );
                }
    ...
    public Position3D getAttachmentWorldPos(String attachmentName, Position3D pos) {
        if (this.legsSprite != null && this.legsSprite.modelSlot != null && this.legsSprite.modelSlot.model != null) {
            ...
            if (attach == null) {
                ...
                Vector3f translation = tempVector3fTL.get();
                xfrm.getTranslation(translation);
                BaseVehicle.releaseMatrix4f(xfrm);
                pos.x = translation.x;
                pos.y = translation.z;
                pos.z = PZMath.fastfloor(translation.y) + (translation.y + 32.0F) % 1.0F / 2.44949F;
                return pos;
            }
        } else {
            return null;
