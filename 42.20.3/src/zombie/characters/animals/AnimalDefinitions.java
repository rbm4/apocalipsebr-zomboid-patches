...
public class AnimalDefinitions {
    ...
    public static volatile HashMap<String, AnimalDefinitions> animalDefs;

    public static HashMap<String, AnimalDefinitions> getAnimalDefs() {
        HashMap<String, AnimalDefinitions> defs = animalDefs;
        if (defs == null) {
            synchronized (DEFINITIONS_LOCK) {
                defs = animalDefs;
                if (defs == null) {
                    loadAnimalDefinitionsLocked();
                    defs = animalDefs;
                }
            }
        }
    ...
    public static ArrayList<AnimalDefinitions> getAnimalDefsArray() {
        ...
        HashMap<String, AnimalDefinitions> defs = getAnimalDefs();

        for (AnimalDefinitions def : defs.values()) {
            result.add(def);
        }
    ...
    private static void loadAnimalDefinitionsLocked() {
        DebugType.General.println("loadAnimalDefinitions start");
        AnimalGenomeDefinitions.getFullGenomeDefs();

        HashMap<String, AnimalDefinitions> loadedDefs = new HashMap<>();
        ...
        if (definitions != null) {
            ...
            while (iterator.advance()) {
                ...
                loadedDefs.put(def.animalTypeStr, def);
            }

            for (AnimalDefinitions def : loadedDefs.values()) {
    ...
    public static AnimalDefinitions getDef(IsoAnimal animal) {
        return animal == null ? null : getDef(animal.getAnimalType());
    }

    public static AnimalDefinitions getDef(String animalType) {
        return animalType == null ? null : getAnimalDefs().get(animalType);
    }
