...
public final class AnimalManagerMain {
    ...
    void saveRealAnimals() {
        ...
        for (IsoMovingObject object : IsoWorld.instance.currentCell.getObjectListSnapshot()) {
            if (object instanceof IsoAnimal animal) {
                if (animal.getData() == null || AnimalDefinitions.getDef(animal.getAnimalType()) == null) {
                    DebugType.Animal.debugln(
                        "Skipping incomplete animal during saveRealAnimals id=%d type=%s",
                        animal.getAnimalID(),
                        String.valueOf(animal.getAnimalType())
                    );
                } else {
