...
public class AnimalInstanceManager {
    ...
    // ApocBR: allocate/check/put was a check-then-act sequence on shared state. With parallel
    // chunk loading (ServerChunkLoader.LOAD_WORKERS) IsoAnimal.load() -> init() runs on several
    // threads, so two animals could end up with the same online ID.
    ...
    public synchronized void add(IsoAnimal animal, short onlineID) {
    ...
    public synchronized void remove(IsoAnimal animal) {
