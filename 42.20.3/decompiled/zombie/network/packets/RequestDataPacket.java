...
public class RequestDataPacket implements INetworkPacket {
    ...
    public void setData(Object... values) {
        this.setRequest((RequestDataPacket.RequestID)values[0]);
    }
    ...
    private void doProcessRequest(UdpConnection connection) {
        ...
        if (this.id == RequestDataPacket.RequestID.PlayerVisited) {
            this.allocateLargeFileBB();
            largeFileBb.clear();
    ...
    private void doProcessData(ByteBufferReader bb) {
        ...
        if (this.id == RequestDataPacket.RequestID.ZombieOutfitDescriptors) {
            try {
                ...
                DebugType.Multiplayer.printException(var7, "PersistentOutfits loading IO error", LogSeverity.Error);
                ExceptionLogger.logException(var7);
            } catch (Exception var8) {
                DebugType.Multiplayer.printException(var8, "PersistentOutfits loading error", LogSeverity.Error);
            }
        }
        ...
        if (this.id == RequestDataPacket.RequestID.PlayerZombieDescriptors) {
            try {
                ...
                DebugType.Multiplayer.printException(var6, "Player zombie descriptors loading error", LogSeverity.Error);
        ...
        if (this.id == RequestDataPacket.RequestID.RadioData) {
            try {
                ...
                DebugType.Multiplayer.printException(var5, "Radio data loading error", LogSeverity.Error);
        ...
        if (this.id == RequestDataPacket.RequestID.WorldMap) {
            try {
                ...
                DebugType.Multiplayer.printException(var4, "Shared map symbols loading error", LogSeverity.Error);
        ...
        if (this.id == RequestDataPacket.RequestID.PlayerVisited) {
            try {
                ...
                ExceptionLogger.logException(var3, "Visited map data loading error", DebugType.Multiplayer, LogSeverity.Error);
            }
        }
    ...
    public static void requestFromServer(RequestDataPacket.RequestID id) {
        INetworkPacket.send(PacketTypes.PacketType.RequestData, id);
    }
