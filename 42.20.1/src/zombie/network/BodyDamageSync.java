...
public class BodyDamageSync {
    ...
    private static final int APocBRUpdateIntervalTicks = Math.max(1, Integer.getInteger("apocbr.bodyDamageSyncUpdateIntervalTicks", 5));
    ...
    public void startSendingUpdates(short localId, short remoteID) {
        if (GameServer.server) {
            ...
            BodyDamageSync.Updater updaterx = new BodyDamageSync.Updater();
            updaterx.localId = localId;
            updaterx.remoteId = remoteID;
            updaterx.bdLocal = player.getBodyDamage();
            updaterx.bdSent = new BodyDamage(player);
            this.updaters.add(updaterx);
        }
    }
