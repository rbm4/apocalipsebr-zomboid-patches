...
public class SyncItemActivatedPacket implements INetworkPacket {
    ...
    public boolean isRelevant(UdpConnection connection) {
        ...
        if (player == null) {
            ...
            DebugType.Multiplayer.warn("[SyncItemActivated] not relevant null player square");
            return false;
        } else if (!connection.isRelevantTo(player.square.x, player.square.y)) {
            DebugType.Multiplayer
                .noise("[SyncItemActivated] not relevant client isoPlayer[x,y]=" + player.getX() + "," + player.getY());
            return false;
        } else {
            return true;
        }
