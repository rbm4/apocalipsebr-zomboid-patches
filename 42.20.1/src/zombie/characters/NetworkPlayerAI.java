...
public class NetworkPlayerAI extends NetworkCharacterAI {
    ...
    private void setMoving(Prediction prediction) {
        ...
        if (!Float.isFinite(movement.x) || !Float.isFinite(movement.y) || movement.getLengthSquared() <= 1.0E-6F) {
            this.setStatic(prediction, this.player);
            prediction.moveDirection = 0.0F;
        ...
        if (!Float.isFinite(timeDelta) || timeDelta <= 1.0E-6F) {
            this.setStatic(prediction, this.player);
            prediction.moveDirection = 0.0F;
            prediction.distance = 0;
            return;
        ...
        float timeDeltaDivider = 1.0F / timeDelta;
        ...
        if (!Float.isFinite(timeScale) || !Float.isFinite(distance) || !Float.isFinite(speed) || distance > 32.0F) {
            this.setStatic(prediction, this.player);
            prediction.moveDirection = 0.0F;
            prediction.distance = 0;
            return;
