...
public final class IsoGridSquare {
    ...
    // purely as a single-threaded-era micro-optimization to avoid allocating a Vector2 per square
    // per frame. Now that CalcVisibility() can run concurrently across multiple ServerLOS worker
    // threads (one per player), a single shared instance would let two threads clobber each
    // other's in-flight calculation. ThreadLocal preserves the "no per-call allocation" property
    // while making it safe for concurrent callers.
        ...
        if (tex_1 != null
            ...
            try {
                ...
                if (dir == IsoDirections.W || dir == IsoDirections.NW) {
                    ...
                    SpriteRenderer.WallShaderTexRender wallShaderTexRenderx;
                    if (dir == IsoDirections.W) {
                        wallShaderTexRenderx = SpriteRenderer.WallShaderTexRender.All;
                    } else {
                        wallShaderTexRenderx = SpriteRenderer.WallShaderTexRender.LeftOnly;
                    }

                    SpriteRenderer.instance.setExtraWallShaderParams(wallShaderTexRenderx);
                    if (PerformanceSettings.fboRenderChunk && FBORenderCell.instance.renderWindowFrameOutline) {
                    ...
                    if (PerformanceSettings.fboRenderChunk && !FBORenderCell.instance.renderWindowFrameOutline) {
                        ...
                        this.DoCutawayShaderAttached(
                            obj, dir, tex2, noWallLighting, lightInfo, cutawayWX, cutawayYAdjustedx, 66, 226, coll2, colu2, colu, coll, wallShaderTexRenderx
                        );
                        CutawayAttachedModifier.instance.setDir(null);
                    }
            ...
            try {
                ...
                if (texture != null) {
                    ...
                    if (dir == IsoDirections.W || dir == IsoDirections.NW) {
                        ...
                        coll2 = this.getVertLight(7, playerIndex);
                        if (sprite.isWallSE()) {
                            SpriteRenderer.instance.setCutawayTexture(tex2, cutawaySEXx + (int)xOffset, 0 + (int)offsetY, 6 - wOffset, 196 - hOffset);
                        } else {
                    ...
                    SpriteRenderer.instance.setCutawayTexture(tex2, cutawaySEXxx + (int)xOffset, 0 + (int)offsetY, 6 - wOffset, 196 - hOffset);
                    Texture depthTexture = TileDepthMapManager.instance.getTextureForPreset(TileDepthMapManager.TileDepthPreset.SEWall);
                    SpriteRenderer.instance.setCutawayTexture2(depthTexture, 0, 0, 0, 0);
                    SpriteRenderer.instance.setExtraWallShaderParams(SpriteRenderer.WallShaderTexRender.All);
                    colu = this.getVertLight(0, playerIndex);
        ...
        if (!GameClient.client) {
            ...
            if (!this.getProperties().has(IsoFlagType.burntOut)) {
                ...
                for (int n = 0; n < this.objects.size(); n++) {
                    ...
                    if (recursive) {
                        IsoObjectUtils.safelyBurnTileObjectFromSquare(objx);
                    }
                    ...
                    if (objx.getSprite() != null
                        ...
                        && objx.getSprite().firerequirement < this.FIRE_IMMUNE_THRESHOLD
                        && !IsoCell.isBasementWallAdjacentToTheVoid_North(objx)
                        && !IsoCell.isBasementWallAdjacentToTheVoid_West(objx)) {
                        if (objx instanceof IsoThumpable && objx.getSprite().burntTile != null) {
                            ...
                            if (GameServer.server) {
                                objx.sendObjectChange(IsoObjectChange.REPLACE_WITH, "object", replace);
                            }

                            objx.removeFromWorld();
                            this.objects.set(n, replace);
                        } else if (objx.getSprite().burntTile == null && objx.getType() != IsoObjectType.tree) {
                            if (!(objx instanceof IsoTrap)) {
                                            ...
                                            if (objx.getProperties().has(IsoFlagType.WindowW)) {
                                                ...
                                                if (objx.getContainer() != null && objx.getContainer().getItems() != null) {
                                                    for (int i = 0; i < objx.getContainer().getItems().size(); i++) {
                                                        InventoryItem item = objx.getContainer().getItems().get(i);
                                                    ...
                                                    if (objx.getSprite().getName().startsWith("roofs_")) {
                                                    ...
                                                    } else if (!objx.getSprite().getName().startsWith("fencing_")
                                                            && !objx.getSprite().getName().startsWith("fixtures_railings_")
                                                        || !objx.getSprite().getProperties().has(IsoFlagType.HoppableN)
                                                            && !objx.getSprite().getProperties().has(IsoFlagType.HoppableW)) {
                                                        if (objx.getSprite().getProperties().has(IsoFlagType.attachedN)
                                                            || objx.getSprite().getProperties().has(IsoFlagType.attachedW)
                                                        ...
                                                        if (objx instanceof IsoThumpable || IsoObjectUtils.isObjectMultiSquare(objx)) {
                                                            objx.sprite = IsoSpriteManager.instance.getSprite("floors_burnt_01_" + Rand.Next(1, 2));
                                                            if (objx.getOverlaySprite() != null) {
                                                                objx.setOverlaySprite(null);
                                                            }
                                                        }
                                                    } else if (objx.getSprite().getProperties().has(IsoFlagType.transparentW)
                            ...
                            objx.sprite = objx.getSprite().burntTile != null
                                ? IsoSpriteManager.instance.getSprite(objx.getSprite().burntTile)
                                        ...
                                        if (gridSquare.y == this.y
                                            ...
                                            if (gridSquare.x != this.x && gridSquare.z == this.z && gridSquare.has(IsoObjectType.stairsMN)) {
                                                ...
                                                if (gridSquare.z < this.z) {
                                                    if (gridSquare.y > this.y
                                                        && gridSquare.x == this.x
                                                        && gridSquare.hasSlopedSurfaceToLevelAbove(IsoDirections.N)) {
                                                        return false;
                                                    }

                                                    if (gridSquare.y < this.y
                                                        && gridSquare.x == this.x
                                                        && gridSquare.hasSlopedSurfaceToLevelAbove(IsoDirections.S)) {
                                                        return false;
                                                    }

                                                    if (gridSquare.x > this.x
                                                        && gridSquare.y == this.y
                                                        && gridSquare.hasSlopedSurfaceToLevelAbove(IsoDirections.W)) {
                                                        return false;
                                                    }

                                                    if (gridSquare.x < this.x
                                                        && gridSquare.y == this.y
                                                        && gridSquare.hasSlopedSurfaceToLevelAbove(IsoDirections.E)) {
                                                        return false;
                                                    }
                                                }
    ...
    public void RecalcProperties() {
        ...
        if (this.roomId == -1L && !this.haveRoof) {
            ...
            try {
                ...
                DebugType.General.printException(var16, LogSeverity.Error);
            }
        }
        ...
        if (!DebugOptions.instance.terrain.renderTiles.isoGridSquare.renderMinusFloor.getValue()) {
            ...
            for (int n = start; doSE ? n >= end : n <= end; n += doSE ? -1 : 1) {
                ...
                if ((!vegitationRender || bGrassEtc && this.flattenGrassEtc) && (vegitationRender || !bGrassEtc || !this.flattenGrassEtc)) {
                    ...
                    boolean isOpenDoor = objx instanceof IsoDoor isoDoor && isoDoor.isOpen() || objx instanceof IsoThumpable isoThumpable && isoThumpable.open;
                    ...
                    if (PerformanceSettings.fboRenderChunk) {
                        ObjectRenderLayer renderLayer = objx.getRenderInfo(playerIndex).layer;
                    ...
                    if (bDoIt) {
                        ...
                        if (objx.sprite == null || !isWestDoorOrWall && !isNorthDoorOrWall) {
                            ...
                            } else if (objx.sprite.getTileType() == IsoObjectType.doorFrW || t == IsoObjectType.doorW || objx.sprite.cutW) {
    ...
    public void CalcVisibility(int playerIndex, IsoGameCharacter isoGameCharacter, VisibilityData visibilityData) {
        ...
        if (!GameServer.server && isoGameCharacter.isDead() && isoGameCharacter.reanimatedCorpse == null) {
            ...
            if (currentSquare != null) {
                ...
                if (chk != null) {
                    // ApocBR: thread-local scratch buffers, see tempoTL/tempo2TL declaration.
    ...
    public DeviceData getDeviceData() {
        ...
        if (vehicle != null) {
            ...
            for (int ix = 0; ix < this.objects.size(); ix++) {
                if (this.objects.get(ix) instanceof IsoWaveSignal isoWaveSignal) {
    ...
    // ApocBR: takes the player directly instead of resolving it through the shared
    // IsoPlayer.players[] slot array. Callers that already hold a direct player reference
    // (e.g. ServerLOS, which computes LOS for multiple players concurrently on separate
    // worker threads) should use this overload to avoid touching that shared global state.
    ...
    public boolean checkHaveGrass() {
        ...
        if (floor == null) {
            return false;
        } else if (floor.getSprite() != null && !floor.getProperties().has(IsoPropertyType.GRASS_FLOOR)) {
            return false;
        } else {
    ...
    public IsoGridSquareCollisionData getFirstBlocking(
        ...
        if (x >= -1 && x <= 1 && y >= -1 && y <= 1 && z >= -1 && z <= 1) {
            ...
            if (x != 0 && y != 0 && specialDiag) {
                ...
                LosUtil.TestResults var18 = !getMatrixBit(this.visionMatrix, x + 1, y + 1, z + 1) ? ret : LosUtil.TestResults.Blocked;
                isoGridSquareCollisionData.testResults = var18;
                return isoGridSquareCollisionData;
            }
        } else {
            isoGridSquareCollisionData.testResults = LosUtil.TestResults.Blocked;
