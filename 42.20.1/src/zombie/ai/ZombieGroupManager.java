...
public final class ZombieGroupManager {
    ...
    private final Map<Long, ArrayList<ZombieGroup>> groupGrid = new HashMap<>();
    private final ArrayList<ZombieGroup> groupCandidates = new ArrayList<>();
    private final Vector2 tempVec2 = new Vector2();
    private final Vector3 tempVec3 = new Vector3();
    private float tickCount = 30.0F;
    private static final int GROUP_GRID_CELL_SIZE = 32;
    ...
    private void indexGroup(ZombieGroup group) {
        if (!group.isEmpty() && group.getLeader() != null) {
            int cellX = cellFor(group.getLeader().getX());
            int cellY = cellFor(group.getLeader().getY());
            int z = PZMath.fastfloor(group.getLeader().getZ());
            this.groupGrid.computeIfAbsent(key(cellX, cellY, z), ignored -> new ArrayList<>()).add(group);
        }
    }

    private ArrayList<ZombieGroup> getGroupsNear(float x, float y, float z, int radius) {
        this.groupCandidates.clear();
        int minCellX = cellFor(x - radius);
        int maxCellX = cellFor(x + radius);
        int minCellY = cellFor(y - radius);
        int maxCellY = cellFor(y + radius);
        int iz = PZMath.fastfloor(z);

        for (int cx = minCellX; cx <= maxCellX; cx++) {
            for (int cy = minCellY; cy <= maxCellY; cy++) {
                ArrayList<ZombieGroup> groups = this.groupGrid.get(key(cx, cy, iz));
                if (groups != null) {
                    this.groupCandidates.addAll(groups);
                }
            }
        }
    ...
    private static int cellFor(float value) {
        return PZMath.fastfloor(value / GROUP_GRID_CELL_SIZE);
    }
