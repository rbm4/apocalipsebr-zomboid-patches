...
public final class BaseVehicle extends IsoMovingObject implements Thumpable, IFMODParameterUpdater, IPositional, VehicleSoundOwner {
    ...
    public static final int ENGINE_SOUND_RADIUS = 100;
    ...
    // ApocBR: Vehicle performance optimizations
    private int animFreezeFrames = 0;
    private int deviceUpdateTick = 0;
    private static final int ANIM_FREEZE_DELAY = 20;
    private static final int ANIM_FREEZE_SPREAD = 8;
    private static final int DEVICE_UPDATE_SPREAD = Integer.getInteger("apocbr.vehicle.deviceUpdateSpread", 4);
    ...
    public int getEngineCondition() {
        ...
        return part == null ? 0 : part.getCondition();
    }
    ...
    public boolean isEngineSounding() {
        if (!this.isListenerInRange(100.0F)) {
            return false;
        } else {
    ...
    public boolean isAlarmSounding() {
        return !this.isListenerInRange(500.0F) ? false : this.soundAlarmOn;
    }
    ...
    private void updateImportantAreas() {
        if (GameServer.server) {
            if (this.isAlarmSounding() || this.isEngineSounding() || this.isSirenSounding()) {
    ...
    private void updateAnimationPlayer(AnimationPlayer animPlayer, VehiclePart part) {
        if (animPlayer != null && animPlayer.isReady()) {
            ...
            // ApocBR Patch B: dirty-flag animation freeze for static parked vehicles.
            // The part==null call is the main body animator - it fires first in postupdate()
            // exactly once per cycle, so we use it as the single point to advance the counter.
            // FIX: replaced isAtRest() (which fails for outdoor tiles lacking a floor object)
            // with a direct driver/engine check that reliably identifies parked vehicles.
            if (part == null) {
                boolean isStaticVehicle = this.getDriver() == null
                    && this.engineState == BaseVehicle.engineStateTypes.Idle;
                if (isStaticVehicle) {
                    if (this.animFreezeFrames < 10000) this.animFreezeFrames++;
                } else {
                    this.animFreezeFrames = 0;
                }
            }
            // Allow full-rate animation while warming up, on any active track (door/window
            // animations), or on the periodic refresh tick to keep the frozen pose stable.
            boolean hasActiveTracks = multiTrack.getTrackCount() > 0;
            boolean shouldAnimate = this.animFreezeFrames < ANIM_FREEZE_DELAY
                || hasActiveTracks
                || (this.animFreezeFrames % ANIM_FREEZE_SPREAD == 0);
            if (shouldAnimate) {
    ...
    private void renderAuthorizations() {
        ...
        TextManager.instance.DrawString(sx, sy + (dy = var26 + fontHgt), String.format("Engine speed: %.3f", this.engineSpeed), r, g, b, a);
        float var28;
    ...
    public void updateSounds() {
        ...
        if (closestListenerDistSq == Float.MAX_VALUE) {
            ...
            if (!GameServer.server) {
                ...
                if (closestListenerDistSq > 10000.0F) {
                    this.stopEngineSounds();
    ...
    public void updateParts() {
        if (!GameClient.client) {
            ...
            // Vanilla: update signal devices every frame for active vehicles
            // (those going through updateParts() have alarm/mechUI/needsUpdate active).
            for (int i = 0; i < this.getPartCount(); i++) {
    ...
    public void drainBatteryUpdateHack() {
        ...
        if (!engineRunning) {
            ...
            // ApocBR Patch A (FIXED): round-robin updateSignalDevice for parked engine-off vehicles.
            // updateParts() is NOT called for parked vehicles (requires needPartsUpdate/alarm/mechUI).
            // drainBatteryUpdateHack() IS the correct code path for parked vehicles.
            if (GameClient.client) {
                this.deviceUpdateTick++;
                if ((this.deviceUpdateTick % DEVICE_UPDATE_SPREAD) == ((this.vehicleId & 0xFFFF) % DEVICE_UPDATE_SPREAD)) {
                    for (int i = 0; i < this.getPartCount(); i++) {
    ...
    public boolean isSirenSounding() {
        return this.getLightbarSirenModeObject().isEnable() && this.getBatteryCharge() > 0.0F;
    }
