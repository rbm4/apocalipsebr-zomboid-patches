...
// ApocBR patch: force-enable Threading.Animation, Threading.World, Threading.Ambient.
// These are non-debug options the devs built but left defaulting to false.
// Defaults changed to true below; load() also force-sets them after reading
// debug-options.ini so a pre-existing cached ini cannot re-disable them.
...
public final class DebugOptions implements IDebugOptionGroup {
    ...
    // This is the primary fix for CPU-bound FPS drops with 30+ parked vehicles in view.
    ...
    private void onTrigger_SetDebugOptions(String entryKey) {
        try {
            ...
            if (optionsXml.setDebugMode) {
                DebugType.General.println("DebugMode: %s", optionsXml.debugMode ? "ON" : "OFF");
